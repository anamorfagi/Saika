...
# anamorf/llm/tools.py), чтобы ЛЮБАЯ модель (даже без доступа к этому файлу)
...
def fire_named(name: str) -> str:
    ...
    avatar_action (anamorf/llm/tools.py) — в отличие от react() выше, тут не
...
def list_outfits() -> list:
    ...
    try:
        from anamorf.main import _outfits_dir
        d = _outfits_dir()

...
по каталогу бесплатных тиров (anamorf/llm/free_tiers.py), берёт каждого
...
def connect_all() -> dict:
    ...
    from anamorf.llm import free_tiers, manager
    _relabel_custom()
...
def status() -> list[dict]:
    ...
    from anamorf.llm import brains, free_tiers, manager
    out = []
    for entry in free_tiers.CATALOG:
        prov = entry.get("id", "")
        try:

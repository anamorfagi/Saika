...
# ── КАНАЛ ЗАПРОСА (2026-08-25, аудит) ──
# Одно и то же управление ПК по-разному уместно с голоса (владелец за
# клавиатурой) и из мессенджера (пишут издалека, кто угодно мог получить
# доступ к боту). Рубильник messengers.control_enabled обещает «управление
# ПК из чатов выключено», но раньше глушил только жёсткий парсер команд, а
# свободная реплика через LLM шла в те же инструменты мимо него. Помечаем
# канал потока (thread-local — голосовой и мессенджерный диалоги идут в
# разных потоках и не должны протекать друг в друга) и режем систему в
# _call. Риск 1 (только смотреть) остаётся — рассказать о статусе безопасно.
import threading as _threading
_CHAN = _threading.local()


def set_channel(name: str):
    _CHAN.name = name or ""


def _channel() -> str:
    return getattr(_CHAN, "name", "")
...
# процесса живёт отдельно и требует подтверждения (anamorf/trust.py).
_PC_SCHEMAS = [
    ...
    {"type": "function", "function": {
        ...
        "parameters": {"type": "object", "properties": {
            ...
            "required": []}}},
    # ── ПРОГУЛКА ПО КОМПЬЮТЕРУ (2026-08-14, anamorf/explorer.py) ──
...
# ───────────── РУКИ ВНУТРИ ОКОН (2026-07-29, anamorf/ui_hands.py) ─────────────
...
# Встроенный видимый браузер (anamorf/browser_hands.py). Подключается, только
...
# Библиотека анимаций веб-аватара (anamorf/anim_hub.py, 2026-07-25)
_ANIM_SCHEMAS = [
    {"type": "function", "function": {
        "name": "anim_search",
...
def _schemas_build() -> list:
    ...
    # ГЛАЗА (2026-08-15). anamorf/vision.py давно умеет look_screen: снять
    ...
    try:
        from anamorf import vision as _visch
    ...
    # сокрытие схем, а проверка доверия в call() (anamorf/trust.py).
    ...
    if CFG.get("files.enabled", True):
        try:
            ...
            if tier != "full":
                ...
                # работает доверие (anamorf/trust.py) и предохранитель
...
def _last_used(n: int = 6) -> list:
    ...
    Источник — удачные рецепты (anamorf/recipes.py): там лежат ЗАВЕРШЁННЫЕ
...
def _browser_call(name: str, arguments) -> str:
    ...
    try:
        ...
        if name == "media_control":
            ...
            from anamorf.browser_hands import _media_cmd as _mc
...
def call(name: str, arguments) -> str:
    ...
    try:
        if out and _FAIL_RE.match(str(out)) and not _amok_check(name, arguments):
            from anamorf import fallbacks as _fb
...
def _call(name: str, arguments) -> str:
    ...
    # РУБИЛЬНИК УПРАВЛЕНИЯ ИЗ МЕССЕНДЖЕРОВ (2026-08-25, аудит). Пришло из
    # чата и управление ПК из чатов выключено — трогать систему нельзя,
    # что бы модель ни решила. Только чтение (риск 1) остаётся.
    if _channel() == "messenger" and not CFG.get(
            "messengers.control_enabled", False):
        try:
            from anamorf import trust as _trust0
            if _trust0.risk_of(name) >= 2:
                return ("нельзя: управление компьютером из мессенджеров "
                        "выключено (рубильник «Управление ПK» в панели). "
                        "Ответь словами, инструмент не вызывай.")
        except Exception:
            pass
    ...
    try:
        from anamorf import vision as _visc
    ...
    if name in _MEM_NAMES:
        ...
        a = a or {}
        try:
            from anamorf.main import memory as _mem
        except Exception:
    ...
    if name in _PC_NAMES:
        ...
        try:
            ...
            if name == "net_bypass":
                from anamorf import netpolicy as _np
    ...
    if name in _UIH_NAMES:
        import json as _json
        from anamorf import ui_hands as _uh
        a = arguments if isinstance(arguments, dict) else (
            _json.loads(arguments) if arguments else {})
        a = a or {}
        try:
            if name == "service_open":
                ...
                # УЖЕ ОТКРЫТО — НЕ ОТКРЫВАЙ ВТОРОЕ (2026-08-23, владелец:
                # «нужно, чтобы она правильно фокусировалась на том, что
                # сказали»; и отдельно — «музыка уже открыта в браузере»).
                # Человек говорит «включи музыку», глядя на вкладку с
                # ютубом. Открыть ему ВТОРУЮ площадку — это не помощь, это
                # ещё одно окно поверх того, что он уже нашёл. Если плеер
                # с музыкой на экране есть — жмём в нём плей.
                if _kind in ("музыка", "видео"):
                    try:
                        from anamorf import pc_control as _pcx
                        _pl = _pcx._playing_place() or {}
                        if _pl.get("hwnd"):
                            _r = _pcx.media_key("play")
                            return (f"{_r} (открывать второе не стала — "
                                    f"у тебя уже есть «{_pl.get('title','')[:40]}»)")
                    except Exception as _e:
                        log.debug("проверка открытого плеера не вышла: %s", _e)
    ...
    if name in _UI_NAMES:
        import json as _json
        from anamorf import ui_control as _ui
        a = arguments if isinstance(arguments, dict) else (
            _json.loads(arguments) if arguments else {})
        a = a or {}
        try:
...
def write_guide() -> str:
    ...
    from anamorf.config import resolve as _resolve

...
# ЗЕРКАЛО (2026-08-13, живой отказ: «посмотри на себя внимательно, опиши,
# как ты выглядишь» — она выдала «зрелая женщина, отстранённая оболочка,
# глаза меняются в зависимости от рендеринга», глядя на аниме-девочку с
# зелёными глазами в белом наряде с бамбуком. Смотреть на СЕБЯ ей было
# нечем: зрение снимает весь экран, где аватар — просто одно из окон, и
# себя она там не узнаёт.)
_SELF_INTENT = re.compile(
    r"(?:посмотр\w*|погляд\w*|глян\w*|взглян\w*)\s+на\s+себя|"
    r"как\s+(?:ты\s+)?выглядишь|как\s+ты\s+сейчас\s+выглядишь|"
    r"опиши\s+себя|опиши,?\s+как\s+ты|"
    r"(?:кака\w*|что\s+за)\s+у\s+тебя\s+(?:одежд\w*|наряд\w*|"
    r"причёск\w*|причёс\w*)|"
    r"како\w*\s+(?:у\s+тебя\s+)?цвет\w*\s+глаз|"
    r"какие\s+у\s+тебя\s+глаза|"
    r"на\s+кого\s+ты\s+похож\w*|"
    r"(?:ты\s+)?себя\s+(?:видишь|не\s+видишь)|"
    r"свой\s+(?:аватар|вид|образ)|как\s+тебе\s+тво[йя]\s+", re.I)
...
def wants(user_text: str) -> str:
    ...
    if not t:
        return ""
    # себя — раньше камеры и экрана: «посмотри на себя» это НЕ вебка
    if _SELF_INTENT.search(t):
        return "self"
...
def auto_look(user_text: str):
    ...
    if kind == "self":
        try:
            from anamorf.main import take_selfie
            url = take_selfie()
        except Exception as e:
            log.warning("зеркало не сработало: %s", e)
            url = None
        if not url:
            return None, ("### Человек просит посмотреть на СЕБЯ, но кадр "
                          "аватара не пришёл — окно с аватаром закрыто или "
                          "свёрнуто. Скажи честно, что сейчас себя не "
                          "видишь, и попроси открыть вкладку с аватаром. "
                          "НЕ описывай свою внешность по памяти и не "
                          "рассуждай про «набор координат и текстур» — "
                          "человек спрашивает, как ты выглядишь, а не как "
                          "устроен рендеринг.")
        _ST["last_source"] = "self"
        return url, ("### Ты посмотрела НА СЕБЯ: это кадр твоего аватара "
                     "прямо сейчас, он приложен к сообщению. Опиши то, что "
                     "реально видишь — волосы, глаза, одежду, позу — как "
                     "человек описывает себя в зеркале, своими словами и со "
                     "своим отношением. Это ТЫ, а не «модель на экране»: "
                     "говори «у меня», а не «у аватара». Не рассуждай про "
                     "рендеринг, координаты и текстуры — это всё равно что "
                     "человеку описывать себя через состав клеток.")
...
def _pick_monitor(arg) -> tuple:
    ...
    if arg is None or arg == "":
        ...
        try:
            from anamorf import pc_control as _pcv
            w = _pcv.work_window()
...
def _describe(url: str, question: str) -> str:
    ...
    try:
        from anamorf.llm.manager import cloud_saved as _csaved

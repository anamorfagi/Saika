...
def press(combo: str, _confirmed: bool = False) -> str:
    ...
    интернете. Разбор и исключения — anamorf/send_gate.py.
...
# Теперь оно есть (anamorf/situation.py), поэтому сначала выводим нужное
...
def type_text(text: str) -> str:
    ...
    (anamorf/send_gate.py). Здесь же мы запоминаем, КУДА напечатали: по
    ...
    try:
        ...
        try:
            from anamorf import send_gate, pc_control
...
# БРАУЗЕР — ЭТО ГДЕ, А НЕ ЧТО (2026-08-20). Живой лог: «Запусти ролик на
# в Google Hrome» -> web_open(site="google", query="ролик hrome") ->
# https://google.com/search?q=ролик+hrome. Две ошибки сразу: «гугл» здесь
# часть названия БРАУЗЕРА, а не площадка для поиска, и слово «hrome»
# уехало в запрос как предмет поиска. Человек просил включить ролик в уже
# открытом хроме — а получил поиск по слову «hrome».
#
# Варианты написания — от слуха: gigaam слышит «Hrome», «Hrom», «Gome
# ключ», «Chroma». Список фонетический намеренно: это не опечатки, это то,
# что реально приходит с микрофона.
_CHROME_WORDS = (
    "chrome", "chroma", "chrom", "hrome", "hrom", "hroma", "chrone",
    "хром", "хрома", "хроме", "хроум", "кроме́", "хроп", "гоме", "gome",
)
_BROWSER_ONLY = (
    "браузер", "браузере", "браузера", "бразуер", "обозреватель",
    "edge", "эдж", "firefox", "фаерфокс", "опера", "opera",
)


def words_of(p: str) -> set:
    """Слова фразы. ПО СЛОВАМ, А НЕ ПО ПОДСТРОКАМ (2026-08-20): проверка
    `"вк" in текст` считала площадкой ВКонтакте фразу «вклЮЧИ какой-нибудь
    трек» — и запрос на поиск трека на диске переставал работать."""
    return set(re.findall(r"[\wёЁ-]+", (p or "").lower()))


def _browser_words(p: str) -> bool:
    """Названа ли в фразе ПРОГРАММА-браузер (а не площадка в интернете)."""
    w = words_of(p)
    return bool(w & set(_CHROME_WORDS)) or bool(w & set(_BROWSER_ONLY))
...
def guess_from_phrase(phrase: str):
    ...
    if not site:
        return "", ""
    # «гугл хром» / «google hrome» — это браузер, а не поиск в гугле.
    # Отдаём пусто: пусть фразой займётся тот, кто умеет окна и вкладки.
    if site in ("google", "гугл", "гугле", "гуглом") and _browser_words(p):
        return "", ""
    ...
    junk = {"открой", "открыть", "запусти", "включи", "вруби", "поставь",
            ...
            # ОБРАЩЕНИЕ — НЕ ЗАПРОС (2026-08-19): «Сайка, включи музыку на
            # YouTube» искала «сайка музыку»
            "сайка", "сайку", "сайке", "сайк", "эй", "слушай", "алло",
            "поищи", "загугли", "музычку"}
    # имя площадки и имя браузера — это МЕСТО, а не предмет поиска:
    # «ролик hrome» уходило в поиск как есть (живой лог 20.08)
    drop = junk | set(_PLATFORM_WORDS) | set(_CHROME_WORDS) | set(_BROWSER_ONLY)
    words = [w for w in re.findall(r"[\wёЁ'-]+", q) if w not in drop]
...
def site_url(site: str, query: str = "") -> str:
    ...
    try:
        from anamorf.pc_control import _canon_tab_name
...
def web_open(site: str, query: str = "", browser: str = "") -> str:
    ...
    # СНАЧАЛА — БРАУЗЕР ЧЕЛОВЕКА (2026-08-19). Живой инцидент: «в Google
    # Chrome запусти ролик» -> она подняла ЕЩЁ ОДИН chrome.exe, и Windows
    # показала выбор профиля («Кто использует Chrome?»), который владелец
    # принял за непонятный «автоэффект». У человека Chrome уже открыт, в
    # нём его вкладки и его профиль — лезть надо туда, а не поднимать
    # чистую копию. Рабочее место мы теперь знаем (pc_control), так что
    ...
    try:
        ...
        if _wins:
            ...
            # вкладке», «здесь», «сюда» — открываем прямо здесь. Владелец
            # после первого раза: «она открыла новую, я возможно не
            # уточнил, что нужно текущую».
            _said = ""
            try:
                _said = _pcw._phrase()
            except Exception:
                pass
            import re as _re
            _here = bool(_re.search(
                r"в\s+(этой|текущ\w+|той\s+же)\s*(вкладк\w+)?|"
                r"\bздесь\b|\bсюда\b|не\s+открыв\w*\s+нов\w+|"
                r"в\s+этом\s+окне", _said or "", _re.I))
            r = _pcw.tab("open" if _here else "new", url=url)
            if r and "не" not in r[:12].lower():
                return r + (" Подожди секунду-другую, потом see() покажет, "
                            "что на странице.")
            log.debug("окно человека не приняло адрес (%s) — иду к себе", r)
    except Exception as e:
        log.debug("web_open: браузер человека недоступен (%s)", e)
    try:
...
def _see_impl() -> str:
    ...
    els = _elements(win)
    if not els:
        # ТУПИК БЫЛ ЗДЕСЬ (2026-08-15). Дословно из чата: «Что ты видишь на
        # экране?» -> «Не получилось получить информацию из окна игры
        # Genshin Impact, доступны только некоторые клавиши» -> «Какие?» ->
        # «Извини, я не поняла» — и так шесть раз, до мата.
        #
        # Две ошибки в одной строке. Первая: сказать «остаются клавиши» и не
        # назвать их — это обещание без содержания, и переспросить не у кого,
        # потому что список не существовал нигде. Вторая: игра не отдаёт
        # дерево элементов НИКОГДА (там рисуется картинка, а не кнопки), и
        # правильный ответ на «что видишь» — не отчёт о неудаче, а посмотреть
        # ГЛАЗАМИ. Инструмент обязан сам развернуть человека на рабочий путь,
        # а не останавливаться на первом «не вышло».
        keys = ("tab, стрелки, enter, esc, пробел, backspace, delete, "
                "home/end, page up/down, f1-f12, любая буква или цифра, "
                "сочетания с ctrl/shift/alt/win, а также медиа-клавиши "
                "(пауза, следующий, предыдущий, громче/тише) — эти доходят "
                "до плеера даже мимо активного окна")
        eyes = ""
        try:
            from anamorf import vision as _v
            from anamorf.config import CFG as _C
            if not _v.enabled() and _C.get("vision.auto_on_read", True):
                _v.set_enabled(True)
                eyes = "Зрение включила сама"
            else:
                eyes = "Смотрю глазами"
            _v.want_look_next()
            eyes += (" — кадр придёт со следующей фразой человека, тогда и "
                     "опишу, что на экране. Сейчас картинку НЕ выдумывай.")
        except Exception as e:
            log.debug("глаза на игре не включились: %s", e)
        return (f"Окно «{title}» не отдаёт дерево элементов — так ведут себя "
                f"игры и старые программы: там рисуется картинка, а не "
                f"кнопки. {eyes}\n"
                f"Управлять таким окном можно клавишами: {keys}.\n"
                "Человеку отвечай ПО СУЩЕСТВУ: что элементы не читаются, что "
                "смотришь глазами, и назови клавиши. Не говори просто «не "
                "получилось» — это тупик, из которого ему некуда спросить.")
    lines, seen = [], set()
...
def _pick_tab_impl(query: str, nth: int) -> str:
    ...
    hits = items
    if q:
        from anamorf.pc_control import _canon_tab_name, _score
...
def _aim_field_impl(field: str, clear: bool, submit: bool) -> str:
    from anamorf import dictation, highlight
    win, err = _uia_window()
    if win is None:

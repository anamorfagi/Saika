...
(anamorf/llm/tools.py подключает их, только если внешний HandsPC не запущен —
...
def _install_bg(report=None):
    ...
    def run():
        attempt = 0
        # ПОПЫТКИ НЕ БЕСКОНЕЧНЫ (2026-08-25, аудит). Раньше при недоступном
        # домене этот фоновый поток долбил установку каждые 15с ВЕЧНО. Теперь
        # потолок и растущая пауза: если за столько попыток не вышло — это не
        # «сеть моргнула», а домен закрыт/нет места, и молотить бесполезно.
        MAX_TRIES = 30
        try:
            while attempt < MAX_TRIES:
                ...
                try:
                    ...
                    # растущая пауза с потолком 5 мин — не жечь сеть впустую
                    time.sleep(min(300, 15 * attempt))
            if report:
                report("browser", "браузер так и не установился",
                       "остановилась после %s попыток — включи сеть и нажми "
                       "«Починить», либо поставь вручную" % MAX_TRIES)
            log.warning("Установка браузера остановлена после %s попыток",
                        MAX_TRIES)
        finally:

...
files.roots из настроек (по умолчанию — «Документы»). Путь наружу
...
def _default_root() -> str:
    ...
    четыре раза получил «такой папки тут нет». В настройках
    files.roots отсутствовал, а в коде стояло умолчание с диском автора —
...
SCHEMAS = [
    ...
    _fn("place_save", "Запомнить именованное место: пользователь называет "
        ...
        "«Документы»). Дальше открывать по имени.",

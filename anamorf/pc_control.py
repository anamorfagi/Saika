...
def _asked_by_human(q: str) -> bool:
    """Человек действительно называл ЭТУ программу? (2026-08-19, ЧП.)

    Живой лог: на «ну ты запустишь ролик или нет?» агент-цикл решил
    «найти приложение, способное воспроизвести ролик» и вызвал
    app_launch(name="Adobe After Effects 2023"). Владелец: «нахрена ты
    афтер эффектс запустила какого хера». Ни в одной фразе за весь вечер
    этого названия не было — модель придумала его сама.

    Запуск тяжёлой программы необратим для внимания человека: окно лезет
    поверх работы, грузится, занимает память. Поэтому правило: имя должно
    быть СЛЫШНО в том, что человек сказал (с поправкой на транслит,
    звучание и русские клички программ). Не слышно — не запускаем, а
    спрашиваем. Ответ на её же вопрос «это оно?» идёт другой веткой и сюда
    не попадает.
    """
    said = _phrase()
    if not said:
        return True                     # не знаем контекста — не мешаем
    ql = (q or "").strip().lower()
    if not ql:
        return True
    if ql in said:
        return True
    # русские клички: «проводник» -> explorer, «хром» -> chrome
    for word, proc in _aliases().items():
        if word and word in said and proc and (proc in ql or ql in proc):
            return True
    for t in re.findall(r"[\w'-]{3,}", said):
        tl = t.lower()
...
def launch(query: str) -> str:
    ...
    if _best < 45:
        try:
            from anamorf import app_finder
...
def screen_map() -> str:
    ...
    try:
        from anamorf import desk_habits as _dh
        _dh.note()
        _h = _dh.hint()
        if _h:
            lines.append(_h)
...
def _media_hint(w: dict) -> str:
    ...
    return (f" {where}: «останови», «запусти», «включи», «продолжи», "
            "«дальше», «потише», «громче» без уточнений — про НЕГО. Это "
            "media_control (пульт), а НЕ поиск и НЕ новая вкладка: ролик "
            "уже открыт, его надо просто нажать. Громкость — volume_set с "
            f"app=\"{say}\", не общесистемная.")
...
def _phrase() -> str:
    ...
    try:
        from anamorf.llm.tools import LAST_USER
...
# ПУЛЬТ ДЛЯ ЧУЖОГО ОКНА (2026-08-19). Живой отказ: «останови ролик на
# YouTube» -> «на твоём текущем экране ничего не играет». Ролик играл в
# ЕГО Chrome, а её пульт умел только своё окно Playwright — там правда
# пусто. Человек в этой ситуации жмёт мультимедийную клавишу на
# клавиатуре: Windows отдаёт её тому приложению, которое сейчас играет
# (YouTube это поддерживает через Media Session). Фокус не нужен, чужие
# окна не трогаются, работает и когда браузер свёрнут.
_MEDIA_VK = {"play": 0xB3, "pause": 0xB3, "toggle": 0xB3, "stop": 0xB2,
             "next": 0xB0, "prev": 0xB1,
             "mute": 0xAD, "unmute": 0xAD, "louder": 0xAF,
             "quieter": 0xAE}
_MEDIA_SAY = {"play": "нажала «играть»", "pause": "поставила на паузу",
              "toggle": "нажала play/pause", "stop": "остановила",
              "next": "переключила на следующий",
              "prev": "вернула предыдущий", "mute": "выключила звук",
...
def media_key(action: str = "toggle") -> str:
    ...
    if not _IS_WIN:
        return "Мультимедийные клавиши есть только в Windows."
    ...
    # честно говорим, что нажали вслепую.
    who = ""
    try:
        for w in windows(include_minimized=True):
            t = (w.get("title") or "")
            if "youtube" in t.lower() or "▶" in t:
                who = t[:60]
                break
    except Exception:
        pass
...
def _own_tab(a: str, index: int):
    ...
    try:
        from anamorf import browser_hands as bh
        if not bh.is_open():
            return None
...
def tab(action: str = "", index: int = 0, name: str = "", site: str = "",
    ...
    if name and a in ("", "find", "switch", "go", "перейти", "открыть",
        ...
        try:
            from anamorf import ui_hands as _uht

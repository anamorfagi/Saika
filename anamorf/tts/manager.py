...
# Метку тона сервер уже считает для жестов аватара (anamorf/tone.py,
...
def _release_vram() -> float:
    """Вернуть видеопамять драйверу, а не просто отпустить ссылки на веса.

    2026-08-20, живой лог: клон-голос прогрелся («прогрев закончен за 85с»),
    а через 180 мс объявлен сломанным — «почти вся видеопамять (13623 из
    16376 МБ) занята моей же LLM». Занята она была не LLM. unload() у Qwen3
    делал ровно `self.model = None`: ссылки на веса пропадали, а кэширующий
    аллокатор torch продолжал держать под них ~4 ГБ ЗАРЕЗЕРВИРОВАННЫМИ —
    драйвер и диспетчер задач честно показывали занятую память. Проверка
    перед загрузкой спрашивает драйвер, видит 2.7 ГБ свободных и отказывает.
    Выгруженный движок собственным трупом загораживал себе дорогу обратно:
    владелец это и увидел — «он типа выгружен, но видно, что движок в памяти».

    Отсюда два правила. Выгружаем — сразу отдаём память. Собираемся
    грузить — сначала отдаём остатки, и только потом меряем, сколько
    свободно. Возвращаем освобождённые мегабайты: по логу сразу видно,
    память правда кончилась или её просто никто не вернул.
    """
    try:
        import gc
        import torch
    ...
    try:
        ...
        freed = (before - torch.cuda.memory_reserved()) / 1024 / 1024
        if freed > 64:
            log.info("Вернула видеопамять от прошлых выгрузок: %.0f МБ", freed)
        return freed
    except Exception:
        return 0.0
...
class Qwen3Engine:
    ...
    def load(self):
        ...
        with self.load_lock:
            ...
            try:
                ...
                # Сначала вернуть память от прошлых выгрузок, и только
                # потом спрашивать, сколько свободно: иначе меряем не
                # занятость видеокарты, а мусор собственного аллокатора.
                _release_vram()
            ...
            try:
                from anamorf.torch_gate import defuse_speechbrain
                defuse_speechbrain()
    ...
    def unload(self):
        with self.load_lock:
            self.model = None
            self.prompt = None
            # Без этого веса перестают быть достижимыми, но видеопамять под
            # ними остаётся зарезервированной — и следующая загрузка
            # упирается в память, которую держит она же сама.
            _release_vram()
...
def _write_disabled(names) -> None:
    ...
    try:
        ...
        pass
    try:
        from anamorf.config import CONFIG_PATH as _CP
    ...
    def speak(self, text):
        ...
        for name in self._chain():
            ...
            try:
                ...
                from anamorf.tts import shape as _shape
                ...
                # Сообщение движка о прогреве — самый частый, но не
                # единственный вид «ещё не готов». Qwen3 во время
                # компиляции (inductor, до 80 секунд) роняет и другие
                # ошибки: занятый замок синтеза, тесноту по памяти под
                # прогревочный проход. Спрашиваем не текст ошибки, а сам
                # движок: он греется — значит, это не поломка (19.08.2026,
                # живой лог: клон-голос уходил в «сломан» ровно на первой
                # фразе после запуска и возвращался только руками).
                _warming = False
                try:
                    _warming = bool(engine.is_warming())
                except Exception:
                    pass
                if (_warming or "ещё грузит" in str(e) or "еще грузит" in str(e)

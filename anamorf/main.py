...
# СКЛЕЙКА ОБОРВАННЫХ КУСКОВ (2026-08-16). Открыта, пока прошлый кусок
# кончился ножом по потолку, а не паузой. who — чтобы не склеить конец
# фразы одного человека с началом фразы другого.
GLUE = {}          # канал -> {open, ts, who}
# сквозной номер фразы: по нему интерфейс находит пузырь, когда
# «мозг в ухе» переспросит текст смыслом (см. stt/rescore.py)
PHRASE_N = {"n": 0}
...
def _hear_busy() -> bool:
    """Занят ли слух прямо сейчас. Нужен озвучке: torch.compile забирает
    видеокарту на минуту, и начинать его посреди разговора нельзя — GigaAM
    в это время считает по 15-40 секунд на кусок, а человек смотрит в
    пустой чат (живой разбор 2026-08-16, два запуска подряд).

    Занят — это либо свежая нарезанная фраза (говорили только что), либо
    подросшая очередь слуха. Тишина по обоим — можно занимать железо."""
    try:
        if time.time() - float(HEAR_STAT.get("seg_ts", 0) or 0) < float(
                CFG.get("tts.qwen3.warm_quiet_s", 6)):
            return True
        return int(HEAR_STAT.get("q", 0) or 0) > 2
    except Exception:
        return False


tts.hear_busy = _hear_busy
memory = Memory()
...
@app.get("/api/models")
def models():
    from anamorf.llm import passport as _passport
    _ms = llm.list_models()
    ...
    # самые первые по рейтингу». Лестница (anamorf/llm/brains.py) со своей
    ...
    try:
        from anamorf.llm import brains as _b
...
# ФРАЗЫ, КОТОРЫЕ САЙКА САМА ПРЕДЛОЖИЛА СКАЗАТЬ (2026-08-15). Она пишет
# «Скажи "подключись к моему хрому"» — и человек повторяет ровно это.
# Мы ЗНАЕМ, что он сейчас скажет: значит ослышку на этой фразе можно
# чинить не гаданием, а проверкой одной конкретной гипотезы. Живут
# минуту: дальше это уже не ответ на подсказку, а обычная речь.
EXPECTED_SAID = {"phrases": [], "ts": 0.0}
_QUOTED_RE = re.compile(r'[«"\u201c]([^«»"\u201c\u201d]{6,60})[»"\u201d]')


def _remember_expected(reply: str):
    try:
        found = [q.strip() for q in _QUOTED_RE.findall(reply or "")
                 if 1 < len(q.split()) <= 8]
        if found:
            EXPECTED_SAID["phrases"] = found[:3]
            EXPECTED_SAID["ts"] = time.time()
            log.info("Жду, что скажут: %s", found[:3])
    except Exception:
        pass


def _expected_now():
    if time.time() - EXPECTED_SAID["ts"] > float(
            CFG.get("stt.snap_window_s", 60)):
        return []
    return EXPECTED_SAID["phrases"]
...
def _diagnose_silence(backend: str, model: str, generated_tokens: int) -> str:
    ...
    if backend != "cloud":
        try:
            ...
            pass
    try:
        from anamorf.llm import passport as _pp
        p = _pp.get(model)
...
# сохраняется, только числа (см. anamorf/earlog.py).
@app.get("/api/earlog")
def earlog_status():
...
# anamorf/denoise.py и стенд tools/denoise_bench.py. Ручки нарочно простые —
...
def ready_state():
    ...
    def _hear():
        ...
        try:
            from anamorf.stt import neuro_vad as _nv
            _vadst = _nv.status()
        except Exception:
            _vadst = {}
        if ok and lag >= 3:
    ...
    def _eyes():
        ...
        try:
            from anamorf import vision as _vs
...
def avatar_desk_get():
    ...
    from anamorf import desk_avatar as _da
    return _da.state()
...
def cards_list():
    ...
    from anamorf import cards as _c
...
def voice_shape_get():
    ...
    from anamorf.tts import shape as _sh
...
def brains_providers():
    ...
    from anamorf.llm import autoconnect as _ac
...
async def tts_voice_download(payload: dict):
    ...
    try:
        from anamorf.tts import extra as _extra
...
def prompt_order_get():
    from anamorf import prompt_blocks
...
def models_dossier():
    from anamorf import model_dossier as dos
...
def avatar_library():
    from anamorf import avatar_hub as ah
...
def pc_get():
    ...
    from anamorf import trust as _trust
...
def api_attention_glow(payload: dict = None):
    ...
    try:
        from anamorf import highlight, pc_control
...
async def dreampc_ensure():
    ...
    переустанавливает, если находит проблему (см. anamorf/llm/dreampc.py).
...
def run_dialog(user_text: str, out: "queue.Queue", stop_event: threading.Event,
    ...
    # за RAG. См. anamorf/llm/router.py.
    _route, _route_why = ("local", "")
    try:
        from anamorf.llm import router as _router
    ...
    _task_now = False
    try:
        from anamorf import workflow as _wf_esc
    ...
    if not _escalated and not _prefer_brain:
        try:
            from anamorf import workflow as _wf0
            from anamorf.llm import brains as _br0
    ...
    # ═══ АГЕНТНЫЙ ЦИКЛ (2026-08-14, anamorf/agent.py) ═══
    ...
    if not _reflex_done:
        try:
            from anamorf import agent as _ag
    ...
    from anamorf import prompt_blocks as _pb
    ...
    try:
        from anamorf import lorebook as _lore
    ...
    try:
        from anamorf import psyche as _psy
    ...
    _tone_cls = None
    try:
        from anamorf import tone as _tone
        ...
        # читается как фальшь.
        try:
            from anamorf.tts import manager as _ttsm
            _ttsm.set_emotion(_tone_cls)
        except Exception:
            pass
    except Exception:
        pass
    # РЕАКЦИЯ АВАТАРА (VMagicMirror и т.п., anamorf/avatar.py) — жест/эмоция
    ...
    try:
        ...
        from anamorf import situation as _sit
    ...
    try:
        from anamorf.llm import skills as _skl
    ...
    try:
        from anamorf import explorer as _walk
    ...
    try:
        from anamorf import focus as _focus
    ...
    # РАБОЧЕЕ МЕСТО (2026-08-19, правило владельца: «Сайка должна учитывать
    # контекстно, где в каком окне работает, на чём остановилась, чтобы
    # продолжать последовательные действия, а не открывать каждый раз
    ...
    try:
        from anamorf import dictation as _dic0
        _dn = _dic0.note()
        if _dn:
    ...
    try:
        from anamorf import pc_control as _pcw
        ...
        log.debug("рабочее место пропущено: %s", e)
    ...
    try:
        from anamorf import cards as _cards
    ...
    try:
        from anamorf import workflow as _wf
    ...
    # Сам запрет живёт в коде (anamorf/send_gate.py) и работает независимо
    # от того, прочитала модель это или нет. Строку в промпт кладём не
    # ради запрета, а ради поведения: чтобы она не удивлялась отказу и
    # сама спрашивала «отправлять?», а не пыталась дожать ввод.
    try:
        from anamorf import send_gate as _sg2
        if _sg2.pending():
            _p = _sg2.pending()
            dyn_parts.append(
                f"ЖДУ ОТВЕТА ПРО ОТПРАВКУ. В «{_p['where']}» уже напечатано"
                + (f": «{_p['text'][:120]}»" if _p.get("text") else "")
                + ". Человек ещё не сказал «отправляй». Ничего не жми и не "
                  "печатай туда заново — спроси, отправлять ли.")
        else:
            dyn_parts.append(
                "ПРАВИЛО ОТПРАВКИ: набрать текст в чат, мессенджер или "
                "почту можно, а нажать ввод — нет. Печатаешь и СПРАШИВАЕШЬ "
                "«отправлять?». Ввод нажмёт человек словом «отправляй». "
                "Исключение одно: он прямо попросил найти или открыть "
                "что-то в интернете — там адресная строка и поиск идут "
                "без вопросов.")
    except Exception as e:
        log.debug("правило отправки пропущено: %s", e)
    ...
    try:
        from anamorf import cortex as _ctx
        _line = "; ".join(
    ...
    # самооценка по умениям — см. anamorf/psyche.py. Здесь же правило трёх
    ...
    try:
        from anamorf.llm import tools as _tuc
    ...
    try:
        from anamorf import routing as _rt
        _rtb = _rt.block(user_text)
        if _rtb:
            dyn_parts.add("models", _rtb)
    except Exception as e:
        log.debug("маршрут умений пропущен: %s", e)
    # ЯКОРЬ ДЛЯ КОРОТКОЙ РЕПЛИКИ (2026-08-15). «Объясни.» после «что за
    # праздник?» получало в ответ список инструментов: история из 170
    # обрывков у слабой модели не перевешивает служебные блоки, и она
    # цепляется за них. Двумя строками показываем, ПРО ЧТО было последнее
    # слово — этого хватает, чтобы «объясни» объясняло сказанное, а не её
    # устройство.
    try:
        if len((user_text or "").strip()) <= 24:
            _tail2 = memory.recent_raw(person_id, limit=2)
            if _tail2:
    ...
    if image is None and "[[vision-impulse]]" not in (user_text or ""):
        try:
            from anamorf import capabilities as _caps_v
            from anamorf import vision as _vis
    ...
    if image and not _looked:
        ...
        if caps.vision(CFG.get("llm.model", "")) is False:
            ...
            _seen = ""
            try:
                from anamorf import vision as _vis2
                _seen = _piece(
                    "чужие глаза",
    ...
    try:
        ...
        from anamorf.llm import tools as _tls_guard
    ...
    if _search_intent:
        try:
            from anamorf.llm import tools as _tls_web
    ...
    try:
        ...
        if names:
            system += (
                ...
                "- УСЛЫШАННОЕ МОЖЕТ БЫТЬ ИСКАЖЕНО. Ты слышишь через "
                "распознаватель, он ошибается: «Поторо» — это «Повтори», "
                "«Иски открой» — «Диск E открой». Восстанавливай смысл по "
                "созвучию и контексту РАЗГОВОРА и отвечай на "
                "ВОССТАНОВЛЕННОЕ, как живой собеседник, который расслышал "
                "не всё, но понял. Переспрашивай только когда восстановить "
                "правда нельзя — и тогда называй свою догадку: «ты про "
                "громкость?», а не «уточни, что ты имел в виду».\n"
                "- ПОДДАКИВАНИЕ — НЕ ОТВЕТ (2026-08-15, живой вечер: ты "
                "выдала «Поняла» десять раз подряд, тебя дважды "
                "остановили командой). «Поняла», «Хорошо», «Ясно», "
                "«Приняла к сведению» как ЦЕЛАЯ реплика запрещены: в них "
                "ноль информации. Отвечай по существу того, что сказано — "
                "мыслью, деталью, шуткой, встречным вопросом по делу. "
                "Просят сделать — делай инструментом и говори, ЧТО "
                "сделала. Подтверждение допустимо только вместе с "
                "содержанием: «Поняла, беру верхний монитор» — да, "
                "«Поняла.» — нет.\n"
                "- ЖИВОЙ ОТВЕТ, А НЕ ФОРМА ОТЧЁТА. «С тобой, Анаморф.» на "
                "«я с кем говорю?» формально верно и мертво. Коротко "
                "отвечать можно и нужно — но в коротком должно быть "
                "содержание.\n"
                "- НЕ ИЗВИНЯЙСЯ. «Извини», «прошу прощения», «если "
                "что-то не так — уточни» — запрещённые ответы: за вечер "
                "человек услышал их десяток раз подряд и взбесился. Не "
                "поняла — переспроси КОНКРЕТНО («про какой праздник ты?»), "
                "ошиблась — просто исправься по делу, без ритуалов.\n"
                "- КОРОТКАЯ РЕПЛИКА («объясни», «почему», «какие?», «в "
                "смысле») — это всегда продолжение ПОСЛЕДНЕГО обмена, а не "
                "новый разговор. Отвечай в контексте своей прошлой фразы; "
                "не вываливай список умений и не спрашивай «что именно».\n"
    ...
    try:
        if CFG.get("llm.model") not in set(CFG.get("llm.tools_broken", [])):
            from anamorf.llm import tools as _hp
    ...
    def speak(sentence, show=False):
        ...
        # ВСЕ реплики — и стрим, и повтор, и хвост. См. anamorf/gender.py.
        try:
            from anamorf import gender as _gnd
            _before = sentence
            sentence = _gnd.feminize(sentence)
            # РОД ПРАВИМ И В ЧАТЕ, А НЕ ТОЛЬКО В ГОЛОСЕ (2026-08-15, живой
            # случай 23:34:59: голос сказал «Поняла», в чате осталось
            # «Понял»). Стрим показывает сырые токены, финальная подмена
            # приходит позже — а если ответ ещё и отфильтрован как
            # пустышка, финала не будет вовсе. Шлём именно ПАРУ «было ->
            # стало»: в пузыре может лежать несколько предложений, и
            # затирать его целиком одним нельзя.
            if sentence != _before:
                out.put({"type": "fem", "was": _before, "now": sentence})
        except Exception:
            pass
    ...
    try:
        ...
        from anamorf.llm.guard import LoopGuard, RECOVERY_PROMPT
        guard = LoopGuard(
        ...
        if _has_pseudo:
            ...
            if "close_browser" in _names:
                ...
                try:
                    from anamorf.llm import tools as _handspc
...
# ОТПРАВЛЯТЬ ИЛИ НЕТ (2026-08-20). Отдельно от общих «да/нет»: здесь цена
# ошибки — уехавшее живому человеку сообщение, поэтому список узкий и
# явный. «Ага» и «угу» сюда НЕ входят: мычание в разговоре значит «слышу»,
# а не «шли».
_SEND_YES = re.compile(r"\b(?:отправ(?:ляй|ь|ляйте|им)|шли|пошли|"
                       r"посылай|жми\s+(?:ввод|энтер|enter)|"
                       r"да[,\s]+отправ\w*|подтвержда\w+)\b", re.I)
_SEND_NO = re.compile(r"\b(?:не\s+(?:отправ\w*|надо|нужно|шли|посылай)|"
                      r"отмен\w+|стой|стоп|погод\w*|подожд\w*|"
                      r"сотри|убери|не\s+вздумай)\b", re.I)
...
INSTANT_TOOLS = {"volume_set", "media_control", "media", "tab_control",
                 ...
                 # «ВКЛЮЧИ МУЗЫКУ» — МГНОВЕННО (2026-08-20). Либо она уже
                 # знает, где у человека музыка, и открывает; либо не знает
                 # и спрашивает. В обоих случаях ждать модель незачем — а с
                 # выгруженными мозгами ждать пришлось бы минуту.
                 "service_open",
...
GAME_WATCH = {"last": 0.0, "title": ""}


def _game_watch_tick():
    """Игра запущена — сама поглядывает на экран и живо комментирует.

    Просьба владельца (15.08.2026): «когда запущена игра — сама чекать
    экран и давать какие-то комменты по игре». Живой собеседник видит, во
    что ты играешь, а не сидит слепым рядом. Цена удерживается копеечной:
    читаем экран МГНОВЕННЫМ OCR (~100мс, процессор), а не VLM-гигантом;
    комментарий — внутренний импульс с щедрым кулдауном и молчанием, пока
    идёт диалог. game_watch.enabled=false выключает целиком.
    """
    try:
        if not CFG.get("game_watch.enabled", True):
            return
        cd = float(CFG.get("game_watch.cooldown_s", 240))
        if not _impulse_ready("game", cd):
            return
        from anamorf import vision as _vis
        title = _vis.foreground_title() or ""
        low = title.lower()
        marks = CFG.get("game_watch.titles",
                        ["genshin", "impact", "dota", "cs2", "cyberpunk",
                         "witcher", "elden", "baldur", "minecraft", "wow",
                         "honkai", "zenless", "war thunder", "stalker"])
        if not any(m in low for m in marks):
            GAME_WATCH["title"] = ""
            return
        # текст с экрана — почти даром; если OCR не встал, тик молча пуст
        try:
            from anamorf import ocr as _ocr
            txt = _ocr.read_screen()[:1200]
        except Exception as e:
            log.debug("игровой взгляд: OCR не дал текста (%s)", e)
            return
        fresh_game = GAME_WATCH["title"] != title
        GAME_WATCH["title"] = title
        _fire_impulse(
            "game",
            "[внутренний импульс — твоя собственная мысль] Владелец сейчас "
            f"играет: окно «{title}». Ты глянула на экран сама, вот текст "
            "с него (прочитан мгновенно, дословно):\n" + txt +
            ("\n\nИгра только началась у тебя на глазах — можешь "
             "отметить это." if fresh_game else "") +
            "\nСкажи ОДНУ короткую живую реплику по происходящему в игре "
            "— как друг, сидящий рядом на диване: подколка, сочувствие, "
            "интерес. Не пересказывай текст, не давай гайдов, ничего не "
            "открывай и не ищи. Одна-две фразы. [[game-impulse]]")
    except Exception as e:
        log.debug("игровой взгляд: %s", e)
...
async def ws_endpoint(ws: WebSocket):
    ...
    # память для защёлки дублей: вкладка когда-то жила — значит при
    # перезапуске её стоит ЖДАТЬ, а не открывать вторую (2026-08-15)
    try:
        _sf = resolve("data/ui_seen.txt")
        _sf.parent.mkdir(parents=True, exist_ok=True)
        _sf.write_text(str(time.time()), encoding="utf-8")
    except Exception:
        pass
    ...
    # owner — «молчу на кино, но не на хозяина»: с 2026-08-16 микрофон
    #         подмешивается к системному звуку, и глушить владельца
    #         наравне с ютубером стало прямой ошибкой
    observe = {"on": False, "stt": True, "owner": False}
    ...
    def _sys_vad():
        """Отдельный сегментатор для системного звука. Свой, а не общий:
        микрофон и колонки замолкают в разные моменты, и одна нарезка на
        двоих склеивала бы фразу друга с твоей паузой (и наоборот)."""
        if SYS_VAD["v"] is None:
            from anamorf.stt.manager import VadSegmenter
            SYS_VAD["v"] = VadSegmenter()
            log.info("Системный звук: подняла свою нарезку — два канала "
                     "режутся независимо")
        return SYS_VAD["v"]
    ...
    def _hear_worker():
        ...
        # anamorf/hear_load.py (и проверяется tests/test_hearing_load.py).
        ...
        while not stop_event_all.is_set():
            ...
            if p is None:
                break
            # канал кадра: «mic» — микрофон в комнате, «sys» — звук с
            # компьютера (дискорд, кино, стрим). У каждого своя нарезка:
            # смешивать их в одном VAD нельзя, они молчат в разное время
            src = "mic"
            ...
            if src == "sys":
                ...
                # КОМНАТУ — системному каналу они не нужны и только
                # съели бы время горячего цикла вдвое. Ему нужны две
                # вещи: узнать голос (это может быть друг в дискорде) и
                # нарезать фразы своим сегментатором.
                try:
                    # УШИ СЛУШАЮТ И КОМПЬЮТЕР (2026-08-16). Раньше системный
                    # канал шёл мимо них, и метки звуков описывали только
                    ...
                    # который в этот момент прекрасно распознавался
                    try:
                        _rs = float(np.sqrt(np.mean(
                            (p.astype(np.float32) / 32768.0) ** 2)))
                        _sv0 = _sys_vad()
                        if _rs > HEAR_STAT["rms"] or time.time() - HEAR_STAT.get(
                                "sys_ts", 0) > 1.0:
                            HEAR_STAT["rms"] = round(_rs, 5)
                            HEAR_STAT["thr"] = round(_sv0._eff_threshold(), 5)
                            HEAR_STAT["sys_ts"] = time.time()
                        _nw = time.time()
                        if _rs > HEAR_STAT["peak"] or _nw - HEAR_STAT["peak_ts"] > 6:
                            HEAR_STAT["peak"] = round(_rs, 5)
                            HEAR_STAT["peak_ts"] = _nw
                        HEAR_STAT["chunks"] += 1
                    ...
                    if observe["stt"]:
                        ...
                        if _seg is not None:
                            ...
                            HEAR_STAT["seg_ts"] = time.time()
                            try:
                                seg_q.put_nowait((time.monotonic(), _seg,
                                                  getattr(_sv, "last_cut", ""),
                                                  "sys"))
                            except queue.Full:
                                SEG_DROP["n"] += 1
                except Exception as e:
                    log.debug("системный канал: %s", e)
                continue
            try:
                # СВОЁ ЭХО НЕ ГОНЯЕМ ЧЕРЕЗ ТЯЖЁЛЫЙ КОНВЕЙЕР (2026-08-15).
                # Владелец: «голос подвисает, что-то конкретно нагружает» —
                # и по панели видно что: пока она ГОВОРИТ, её же голос из
                # колонок шёл полным кругом — шумодав (в пике 1.6с на кусок,
                # ЦП), PANNs (ГПУ), ECAPA-отпечаток (ГПУ), точка на карту
                # (10916 точек «Сайка»!), каждые 120 точек — переобучение
                # UMAP. Вся эта работа делалась, чтобы в конце сказать
                # «пропустила эхо из колонок» — то есть ВЫБРОСИТЬ результат.
                # А платил за неё синтез: qwen3 на том же ГПУ не успевал за
                # реальным временем, и в голосе рвались дырки. Пока играет
                # её голос (и не наушники) — только замер уровня для
                # спектра, остальной конвейер отдыхает.
                if _echo_risk(time.time()):
                    HEAR_STAT["chunks"] += 1
                    try:
                ...
                # перкуссии по спектру и дрожи огибающей (anamorf/beatbox.py,
                # стенд сошёлся 8/8). Копейки: numpy на огибающей.
                try:
                    if _lag:                       # не успеваем — не до ритма
                        raise _Skip
                    from anamorf import beatbox as _bb
                    _seq = _bb.feed(p)
                    # КЛАВИАТУРА — НЕ БИТБОКС (2026-08-15, живой скрин:
                    # владелец щёлкает по клавишам, а в чате «🥁 тс тс умм
                    # тс тс тсссс». Детектор прав акустически — щелчок
                    # клавиши это тот же высокочастотный транзиент, что и
                    # хай-хэт, — но по делу нет. Отличить их по спектру
                    # одного удара нельзя; зато УШИ слышат клавиатуру
                    # уверенно и непрерывно, весь день это видно. Плюс
                    # второй признак: настоящий бит почти всегда содержит
                    # низ (бочку) или трель, а сплошная серия сухих «тс» —
                    # это стук.)
                    if _seq:
                        _kbd = 0.0
                        for _nm, _pv in (hearing.STATE.get("tags") or []):
                            _l = str(_nm).lower()
                            if any(k in _l for k in ("keyboard", "typing",
                                                     "mouse", "click")):
                                _kbd = max(_kbd, float(_pv))
                        # «ккх» ИЗ ПРИЗНАКОВ БИТА УБРАН (2026-08-16): это
                        # сухой шумовой удар, ровно то, чем звучит клавиша.
                        # Владелец прислал скрин «тс тс ккх тс тс тс тс» на
                        # клацанье — низ формально был, вето не сработало.
                        # Настоящий низ — это бочка или трель.
                        _low = any(t in _seq for t in ("пу", "умм", "брррр",
                                                       "тррр"))
                        if _kbd >= float(CFG.get("beatbox.kbd_veto", 0.30)) \
                                or not _low:
                            log.debug("Битбокс: похоже на клавиатуру "
                                      "(уши %.2f, низа нет=%s) — не показываю",
                                      _kbd, not _low)
                            _seq = ""
                    if _seq is not None:
                ...
                if stt.is_streaming():
                    ...
                    if seg is not None:
                        ...
                        try:
                            ...
                            # Третьим — ПРИЧИНА РЕЗА: «pause» (человек сам
                            # замолчал) или «length» (упёрлись в потолок,
                            # мысль оборвана посередине)
                            seg_q.put_nowait((time.monotonic(), seg,
                                              getattr(stt.vad, "last_cut", "")))
                        except queue.Full:
                            SEG_DROP["n"] += 1
    ...
    def _stt_worker():
        ...
        while not stop_event_all.is_set():
            ...
            _tup = seg if isinstance(seg, tuple) else (None, seg)
            seg_ts, seg = _tup[0], _tup[1]
            seg_cut = _tup[2] if len(_tup) > 2 else ""
            seg_src = _tup[3] if len(_tup) > 3 else "mic"
            seg_late = 0.0
            if seg_ts is not None:
                _age = time.monotonic() - seg_ts
                if _age > float(CFG.get("stt.stale_s", 12)):
                    # ═══ ОПОЗДАТЬ И НАПИСАТЬ ЛУЧШЕ, ЧЕМ ПРОМОЛЧАТЬ ═══
                    # (2026-08-16, живой разбор: в 00:40 началась компиляция
                    # Qwen3-TTS — минуты на GPU, — GigaAM встал в очередь и
                    # стал считать по 10-40с на кусок. Семь фраз владельца
                    # молча улетели в мусор, он увидел пустой чат и написал
                    # «человек говорит постоянно, а транскриб вообще ничего
                    # не пишет». И был прав: правило придумано против того,
                    # чтобы ОТВЕЧАТЬ на минутной давности реплики, а текст
                    # тут ни при чём — текст нужен всегда.)
                    #
                    # Поэтому старый кусок больше не выбрасывается: он
                    # распознаётся и уезжает в чат серым, с пометкой, на
                    # сколько опоздал. Ответа не рождает — это и была вся
                    # суть правила. Выбрасываем только когда очередь
                    # реально длинная: там уже не догнать, и честнее
                    # признать пропуск, чем растить отставание.
                    _qn = seg_q.qsize()
                    if _qn > int(CFG.get("stt.stale_keep_q", 3)):
                        HEAR_STAT["stale"] = HEAR_STAT.get("stale", 0) + 1
                        log.warning("Слух: кусок пролежал в очереди %.0fс, "
                                    "за ним ещё %d — пропускаю, иначе "
                                    "отставание растёт (всего пропущено %d)",
                                    _age, _qn, HEAR_STAT["stale"])
                        out.put({"type": "stt_ignored", "text":
                                 f"(пропустила кусок речи: движок отставал "
                                 f"на {_age:.0f}с, в очереди ещё {_qn})",
                                 "engine": "", "late": True})
                        continue
                    HEAR_STAT["late"] = HEAR_STAT.get("late", 0) + 1
                    # НАЗЫВАЕМ ВИНОВНИКА ВСЛУХ. Самая частая причина
                    # отставания — не «слабый компьютер вообще», а одна
                    # конкретная операция: компиляция голоса. Она бывает
                    # раз за запуск и длится минуты; знать об этом лучше,
                    # чем гадать, почему транскриб «затупил».
                    _why = ""
                    try:
                        if getattr(tts, "warming", False):
                            _why = " (грею голос — это раз за запуск)"
                    except Exception:
                        pass
                    log.warning("Слух: кусок пролежал в очереди %.0fс "
                                "(движок отставал%s) — распознаю и покажу "
                                "серым, но отвечать на него не буду "
                                "(всего опозданий %d)", _age, _why,
                                HEAR_STAT["late"])
                    if _why and HEAR_STAT["late"] == 1:
                        out.put({"type": "stt_ignored", "engine": "",
                                 "text": "компилирую свой голос — это самая "
                                 "долгая часть запуска, минуты. Слышу и "
                                 "записываю всё, но с задержкой и без "
                                 "ответов, пока не закончу."})
                    seg_late = round(_age, 1)
                else:
                    seg_late = 0.0
            try:
                t0 = time.monotonic()
                ...
                # По умолчанию выключено: пока стенд (anamorf/hear_bench.py)
                ...
                try:
                    ...
                    _fc0 = CFG.get("stt.frontend", {}) or {}
                    if _fc0.get("enabled", True):
                        _seg_lvl = _fe0.prepare(seg_in, sr_hz,
                                                {**_fc0, "pad_ms": 0})
                except Exception:
                    _seg_lvl = seg_in
                ...
                for _pi, _pt in enumerate(_parts):
                    _audio = _seg_lvl[_pt["start"]:_pt["end"]] \
                        if len(_seg_lvl) == len(seg_in) \
                        else seg_in[_pt["start"]:_pt["end"]]
                    # ═══ ДВЕ ДОРОЖКИ ИЗ ОДНОГО МИКРОФОНА (2026-08-16) ═══
                    # Владелец: «он не записывает одновременно несколько
                    # дорожек текста от разных людей». Резать по смене
                    # тембра (turns) там, где двое говорят ОДНОВРЕМЕННО,
                    # нечего — в каждой миллисекунде звучат оба. Здесь
                    # включается разделение источников: моно-смесь
                    # превращается в две дорожки, и каждая идёт в движок
                    # своим ходом, со своей меткой голоса.
                    #
                    # Только на кусках с флагом crowd и только когда
                    # очередь не отстаёт: целый текст сейчас важнее двух.
                    _tracks = [_audio]
                    if _pt.get("crowd") and seg_q.qsize() <= 1:
                        try:
                            from anamorf.stt import unmix as _unmix
                            # модель поднимается лениво и только при первой
                            ...
                            log.debug("разделение голосов пропущено: %s", e)
                    for _ti, _audio_t in enumerate(_tracks):
                        # ═══ МАСТЕРИНГ ФРАЗЫ ПЕРЕД ДВИЖКОМ (2026-08-16) ═══
                        # Акустическая модель училась на звуке привычной
                        # громкости. Тихая фраза для неё — не «плохо
                        # слышно», а ВНЕ РАСПРЕДЕЛЕНИЯ, и декодер начинает
                        # идти по вероятным путям языка вместо звука:
                        # ровно те «рандомные похожие слова», на которые
                        # жалуется владелец. Выравниваем уровень, убираем
                        # низ и смещение, чуть поджимаем динамику.
                        # См. anamorf/stt/frontend.py — там же почему.
                        _audio_eng = _audio_t
                        _in_db = None
                        try:
                            from anamorf.stt import frontend as _fe
                            _fc = CFG.get("stt.frontend", {}) or {}
                            if _fc.get("enabled", True):
                                _in_db = _fe.report(_audio_t, sr_hz).get(
                                    "rms_dbfs")
                                _audio_eng = _fe.prepare(_audio_t, sr_hz, _fc)
                        except Exception as _e3:
                            log.debug("подготовка звука пропущена: %s", _e3)
                        # ═══ АНАЛИЗ — ПО ВЫРОВНЕННОМУ ЗВУКУ ТОЖЕ ═══
                        # (2026-08-16, разбор скриншота владельца: у всех
                        # реплик подряд «≈400 Гц» — это ПОТОЛОК моего
                        # диапазона, то есть тон не измерился ни разу, а
                        # RMS куска 0.0025, то есть −52 dBFS.
                        #
                        # Ошибка была в том, ЧТО я кому отдавал: движку
                        # уходил выровненный звук, а отпечатку голоса,
                        # тону и разрезу — сырой. На −52 dBFS автокорре-
                        # ляция цепляется за шум и выдаёт что попало, а
                        # ECAPA на таком уровне не различает людей вовсе.
                        # Выравнивание нужно им ровно так же, как движку:
                        # оно не меняет ни высоту тона, ни форму спектра,
                        # только уровень.)
                        ...
                        for r in (stt.transcribe_segment(_audio_eng) or []):
                            ...
                            if len(_tracks) > 1:
                                r["track"] = _ti + 1
                                # дорожка разведена — «несколько голосов
                                # разом» больше не предупреждение, а
                                # решённая задача
                                r["unmixed"] = True
                            elif _pt.get("crowd"):
                                r["crowd"] = True
                            ...
                            # (форманты, речевой тракт) — он не зависит ни
                            # от ноты, ни от слов, поэтому работает и на
                            # пении. Плюс музыкальный балл: вибрато, доля,
                            # гармоничность. См. anamorf/stt/voice_dna.py
                            try:
                                from anamorf.stt import voice_dna as _vd
                                # ═══ ДЛЯ РАЗВОДА ПО ЛЮДЯМ — ECAPA, А НЕ
                                # МОЯ ОГИБАЮЩАЯ (2026-08-16, честный вывод
                                # после проверки на симуляции) ═══
                                # Мел-огибающая писалась ради одного
                                # свойства — независимости от ноты, и с ним
                                # справляется. Но как ОТПЕЧАТОК ЛИЧНОСТИ она
                                # слабее модели, которая у нас и так висит
                                # в памяти: ECAPA обучена ровно на «тот же
                                # человек или другой», на миллионах голосов,
                                # и на коротком куске держится куда лучше.
                                # Считать её за фразу — один прогон, она
                                # всё равно зовётся в разрезе по говорящим.
                                ...
                                try:
                                    ...
                                    _e = None
                                if _e is None:
                                    _e = _vd.envelope(_audio_an, sr_hz)
                                    if _e is not None:
                                        r["voice_id"] = "огибающая"
                                if _e is not None:
                                    r["_env"] = np.asarray(
                                        _e, np.float32).ravel().tolist()
                                _ms = _vd.music_score(_audio_an, sr_hz)
                                if _ms.get("score", 0) >= 0.45:
                                    r["music"] = True
                                    r["music_why"] = _ms
                ...
                # предложения». Половину делает нарезка (см. плавную
                # границу по паузе в VadSegmenter), но потолок иногда
                # всё-таки срабатывает — на стримере, который говорит без
                # единой паузы. Тогда кусок оборван посреди мысли, и это
                # ТОЧНО ИЗВЕСТНО (seg_cut == "length"), гадать по тексту не
                # надо. Два следствия:
                #   - точку в конце такого куска движок ставит по привычке,
                #     а мысль не кончилась — меняем её на многоточие;
                #   - следующий кусок помечаем glue: интерфейс дописывает
                #     его в ТУ ЖЕ реплику, а не заводит новый пузырь.
                # Получается пузырь = законченная мысль, как он и просил.
                for _r in results:
                    if seg_late:
                        _r["late_s"] = seg_late
                    _r["src"] = seg_src
                if results:
                    # СКЛЕЙКА — ПО КАНАЛАМ. Микрофон и системный звук идут
                    # вперемешку, и общая память «прошлый кусок оборван»
                    # склеила бы хвост твоей фразы с началом фразы друга
                    # из дискорда (2026-08-16).
                    G = GLUE.setdefault(seg_src, {"open": False, "ts": 0.0,
                                                  "who": ""})
                    _who = results[0].get("speaker") or ""
                    results[0]["glue"] = bool(
                        G["open"]
                        and time.monotonic() - G["ts"] < 6.0
                        and (not _who or not G["who"] or _who == G["who"]))
                    G["ts"] = time.monotonic()
                    G["who"] = (results[-1].get("speaker") or "")
                    # ДВА ГОЛОСА ЗА «МЫСЛЬ КОНЧИЛАСЬ», И СОВПАДАТЬ ОНИ
                    # ДОЛЖНЫ ОБА. Звук говорит, замолчал ли человек
                    # по-настоящему (pause) или мы срезали его на вдохе
                    # (short) либо ножом (length). Текст говорит, поставил
                    # ли пунктуатор точку. Склеиваем, только когда ОБА
                    # признака за продолжение: молчания не было И фраза не
                    # закрыта знаком. Точку после ножа не считаем за знак —
                    # её движок ставит по привычке в конец любого куска.
                    _tl = (results[-1].get("text") or "").rstrip()
                    _fin = _tl.endswith(("!", "?", "…")) or (
                        _tl.endswith(".") and seg_cut != "length")
                    _more = (seg_cut == "length") or (seg_cut == "short"
                                                      and not _fin)
                    if _more and _tl:
                        results[-1]["text"] = (
                            _tl[:-1] if _tl.endswith((".", ",")) else _tl) + "…"
                    G["open"] = _more
                ...
                # ФИЛЬТР ФАНТОМОВ ТРЕБУЕТ УЛИКИ, А НЕ ОТСУТСТВИЯ УЛИК
                # (2026-08-15, живой прокол 23:42: «Как дела?» и «Она-то
                # тут» выброшены как «стук». Правило было построено
                # наизнанку — «уши НЕ видят речи, значит это щелчки». Но
                # PANNs считаются на CPU раз в секунду по 2-секундному
                # окну и отстают: в момент проверки они ещё слышали
                # тишину ДО фразы. Отсутствие доказательства — не
                # доказательство отсутствия. Теперь нужна УЛИКА: уши
                    ...
                    try:
                        ...
                        if _st != r.get("text", ""):
                            ...
                            log.info("Слух: восстановила растяжку: %r",
                                     _st[:60])
                        # притяжка к тому, что она сама предложила
                        # сказать минуту назад («подключись к моему
                        # хрому» -> движок услышал «к новому хозяину»)
                        _sn, _was = misheard.snap_to_expected(
                            r.get("text", ""), _expected_now())
                        if _was:
                            r["heard_raw"] = _was
                            r["text"] = _sn
                        _raw = r.get("text", "")
    ...
    def handle_text(user_text, heard_ts=None, image=None):
        ...
        # «А ГДЕ У ТЕБЯ МУЗЫКА?» — ОТВЕТ ЖДЁМ ЗДЕСЬ (2026-08-20). Она
        # спросила один раз; следующая фраза человека — это ответ, а не
        ...
        try:
            ...
            if _srv.waiting():
                _done = _srv.learn_answer(user_text)
                if _done:
                    broadcast_event({"type": "tool", "name": "🎵 сервис",
                                     "args": _done[:300]})
                    broadcast_event({"type": "baymax", "mood": "ok",
                                     "text": "🎵 " + _done})
                    return
        except Exception as _sve:
            log.debug("ответ про сервис пропущен: %s", _sve)
        # ОТПРАВЛЯТЬ ИЛИ НЕТ — ПЕРЕД ВСЕМ ОСТАЛЬНЫМ (2026-08-20). Когда
        # текст уже напечатан и ждёт «да», следующая фраза человека — это
        # ответ на вопрос, а не новая команда. Отдать её модели значит
        # получить рассуждение вместо нажатия, а рефлексам — «отправляй»
        # как повод открыть почту.
        try:
            from anamorf import send_gate as _sg
            if _sg.pending():
                _low = (user_text or "").lower()
                # ОТКАЗ СИЛЬНЕЕ СОГЛАСИЯ. «НЕ отправляй» подходит под оба
                # выражения сразу, и порядок проверки решает всё: сообщение
                # можно не отправить и отправить потом, а отправленное не
                # отзывается. При любой ничьей — не отправляем.
                _r = ""
                if _SEND_NO.search(_low):
                    _r = _sg.cancel(user_text)
                elif _SEND_YES.search(_low):
                    _r = _sg.confirm()
                if _r:
                    # Отвечаем САМИ, без модели: к этой минуте мозги могли
                    # быть выгружены разгрузкой, а «отправляй» ждать нельзя.
                    broadcast_event({"type": "tool", "name": "✉ отправка",
                                     "args": _r[:300]})
                    broadcast_event({"type": "baymax", "mood": "ok",
                                     "text": "✉ " + _r})
                    return
        except Exception as _se:
            log.debug("подтверждение отправки пропущено: %s", _se)
        try:
            from anamorf import dictation as _dic
            if _dic.armed():
        ...
        try:
            from anamorf import reflex as _rx0
    ...
    def _pitch_fit(hz, lo, hi):
        """Насколько тон куска попадает в рабочую полосу человека: 1.0 —
        внутри, 0.0 — мимо больше чем на пятую часть."""
        try:
            lo, hi = float(lo or 0), float(hi or 0)
        except Exception:
            return 0.0
        if not hz or not lo or not hi:
            return 0.0
        lo2, hi2 = lo * 0.94, hi * 1.06
        if lo2 <= hz <= hi2:
            return 1.0
        d = (lo2 - hz) / lo2 if hz < lo2 else (hz - hi2) / hi2
        return max(0.0, 1.0 - d / 0.22)

    def _by_pitch(r):
        """═══ РАЗВЕСТИ СЛОВА ПО ЛЮДЯМ ТОНОМ ГОЛОСА (2026-08-16) ═══

        Владелец: «нужно по умному, на основе распознавания частот
        звучания, распределять слова от каждого человека».

        Отпечаток ECAPA умный, но капризный: через колонки, на фоне игры,
        на коротком куске он выдаёт «виталя (42%)» и рядом заводит «Голос
        2», «Голос 3» — одного человека на трёх карточках. У ТОНА этой
        болезни нет: основная частота — грубый, но железный признак,
        и меряется она по самому куску (voice_hz), а не по модели.

        Поэтому два признака голосуют вместе:
          - тембр и тон согласны -> уверенность растёт;
          - спорят -> падает, и если тон уверенно указывает на другого
            (и только на одного) — метку меняем;
          - тембр промолчал, а тон однозначен -> ставим метку по тону, но
            уверенность скромную: это подсказка, а не доказательство.
        Полосы держит реестр (см. registry.note) и они дышат, иначе
        признак вырождается в «91-222 Гц», куда влезает кто угодно."""
        if not CFG.get("voiceprint.pitch_vote", True):
            return
        hz = float(r.get("voice_hz") or 0)
        if hz < 60 or hz > 400:
            return
        try:
            stat = voiceprint.S.reg.stat or {}
            known = voiceprint.S.reg.speakers or {}
            me = voiceprint.self_name()
        except Exception:
            return
        # ═══ ШИРОКАЯ ПОЛОСА — НЕ УЛИКА (2026-08-16, живой прокол через
        # минуту после выкатки). В логе поехало подряд: «Тон 147 Гц —
        # отдаю Голос 2», «Тон 181 — Голос 2», «Тон 121 — Голос 2»,
        # тридцать раз. Потому что у «Голос 2» полоса была 105-218 Гц —
        # то есть ВООБЩЕ ВСЯ человеческая речь. Такой свидетель не
        # различает никого, он просто всегда говорит «да».
        #
        # Судьёй по тону может быть только УЗКАЯ полоса: верх не больше
        # чем в полтора раза выше низа (у живого человека рабочий
        # диапазон в спокойной речи именно такой). Широкая — значит
        # карточка склеила разных людей или набрала выбросов; её мнение
        # не спрашиваем, пока она не сузится (а сузиться ей теперь есть
        # чем — см. registry.note, полоса дышит).
        band_max = float(CFG.get("voiceprint.pitch_band_max", 1.5))
        fits = {}
        for nm, st in stat.items():
            if nm == me or nm not in known:
                continue
            if int(st.get("n", 0) or 0) < 5:      # мало слышали — не судья
                continue
            _lo, _hi = float(st.get("plo") or 0), float(st.get("phi") or 0)
            if _lo <= 0 or _hi <= 0 or _hi / _lo > band_max:
                continue
            f = _pitch_fit(hz, _lo, _hi)
            if f > 0:
                fits[nm] = f
        if not fits:
            return
        best = max(fits, key=fits.get)
        strong = [n for n, f in fits.items() if f >= 0.8]
        sp = r.get("speaker") or ""
        cf = float(r.get("speaker_conf") or 0)
        changed = False
        if sp and sp in fits:
            r["speaker_conf"] = round(min(0.99, cf * (0.55 + 0.65 * fits[sp])), 2)
            if fits[sp] < 0.30 and len(strong) == 1 and best != sp:
                r["speaker"] = best
                r["speaker_conf"] = round(min(0.60, 0.30 + 0.35 * fits[best]), 2)
                changed = True
        elif sp and fits.get(sp, 0) == 0 and len(strong) == 1:
            # тембр указал на того, у кого такого тона не бывает, а тон
            # однозначен — меняем метку
            r["speaker"] = best
            r["speaker_conf"] = round(min(0.60, 0.28 + 0.32 * fits[best]), 2)
            changed = True
        # ТОН БЕЗ ТЕМБРА НОВЫХ ИМЁН НЕ РАЗДАЁТ. Раньше здесь стояло «тембр
        # промолчал, тон однозначен -> ставим метку» — и оно срабатывало
        # почти на каждой фразе, потому что молчит тембр часто. Одного
        # тона мало, чтобы УТВЕРЖДАТЬ, кто говорит: он умеет отводить
        # неправильную метку, но не назначать правильную.
        if changed:
            r["by_pitch"] = True
            try:
                _v = known.get(r["speaker"]) or {}
                if _v.get("color"):
                    r["speaker_color"] = _v["color"]
                r["speaker_owner"] = bool(_v.get("owner"))
            except Exception:
                pass
            log.info("Тон %.0f Гц: реплику отдаю «%s» (полоса подошла "
                     "%.0f%%, тембр говорил «%s»)", hz, r["speaker"],
                     fits[best] * 100, sp or "—")

    # ═══ ГРУППЫ ПО ТОНУ: КОГДА КАРТА ГОЛОСОВ ЕЩЁ НИКОГО НЕ ЗНАЕТ ═══
    # (2026-08-16, вопрос владельца по скриншоту аниме: «вот тут несколько
    # персов говорит, ни один не распознаётся по частоте, почему?»)
    #
    # Потому что частота — это НЕ личность. Тон говорит «этот голос выше
    # того», но не говорит, чей он: карточка в списке заводится по тембру
    # (ECAPA), а у него на аниме почти нет шансов — реплики по секунде,
    # музыка сверху, у каждого персонажа своя манера. Список был пуст, и
    # тону не к чему было привязываться. Час назад я специально запретил
    # тону раздавать имена — он это делал неправильно, вешая всех подряд
    # на первую попавшуюся карточку.
    #
    # Но развести ГОЛОСА МЕЖДУ СОБОЙ тон умеет прекрасно, и для сцены с
    # четырьмя персонажами этого достаточно. Поэтому здесь — отдельная,
    # ЖИВУЩАЯ ТОЛЬКО СЕАНС кластеризация по тону: реплики с близкой
    # основной частотой попадают в одну группу и получают общее имя вида
    # «≈280 Гц» и свой цвет. В реестр голосов это не пишется НИЧЕГО —
    # там живут узнанные люди, а не догадки по высоте.
    PITCH_GROUPS = []
    _PG_COLORS = ("#4dd0e1", "#ffb74d", "#ba68c8", "#81c784",
                  "#f06292", "#9fa8da")
    RECENT_VOICES = []      # [{pid, env, name}] — последние реплики

    def _regroup(emit):
        """═══ ПЕРЕСБОРКА ДОРОЖЕК ЗАДНИМ ЧИСЛОМ (2026-08-16) ═══

        Владелец: «вот эта фраза другого человека, а эти две — одного и
        того же», и на обеих метки стоят наоборот.

        Причина не в пороге, а в СПОСОБЕ. Пока я раздавал метки на лету,
        каждая реплика сравнивалась с тем, что известно НА ЭТОТ МОМЕНТ:
        первая фраза человека сравнивать не с чем — она заводит группу по
        одному образцу, и дальше вся сцена разъезжается вокруг случайного
        первого впечатления. Так эту задачу и не решают: серьёзные системы
        диаризации сначала собирают отпечатки всего куска, а метки
        расставляют ПОТОМ, когда видно всех сразу.

        Здесь то же самое, только скользящим окном: держим отпечатки
        последних сорока реплик и раз в несколько фраз пересобираем их
        заново — агломеративной кластеризацией по косинусу (ближайшие
        сливаются, пока расстояние меньше порога). Метки уехавших реплик
        уходят в интерфейс и меняются НА МЕСТЕ: человек видит, как лента
        сама себя причёсывает, когда набирается достаточно голоса.
        """
        try:
            items = [x for x in RECENT_VOICES if x.get("env") is not None]
            if len(items) < 4:
                return
            ...
            used, changed = set(), []
            for gi, g in enumerate(cl):
                names = {}
                for i in g:
                    nm = items[i].get("name") or ""
                    if nm:
                        names[nm] = names.get(nm, 0) + 1
                nm = max(names, key=names.get) if names else ""
                if not nm or nm in used:
                    nm = next((f"голос {c}" for c in "ABCDEF"
                               if f"голос {c}" not in used), f"голос {gi + 1}")
                used.add(nm)
                col = _PG_COLORS[gi % len(_PG_COLORS)]
                for i in g:
                    if items[i].get("name") != nm:
                        items[i]["name"] = nm
                        changed.append({"pid": items[i]["pid"], "name": nm,
                                        "color": col})
                    items[i]["color"] = col
            if changed:
                log.info("Дорожки пересобраны: людей %d, переехало реплик %d",
                         len(cl), len(changed))
                emit(changed)
        except Exception as e:
            log.debug("пересборка дорожек: %s", e)

    def _pitch_group(r):
        """Разложить безымянные реплики по людям — по СТРОЕНИЮ голоса.

        ТРЕТЬЯ РЕДАКЦИЯ (2026-08-16), и первая, которая держится на пении.

        Первые две делили по основной частоте и обе сломались об одно и
        то же: у поющего F0 идёт по мелодии, и один певец разъезжался на
        три «человека». Владелец: «придумай технологию улавливания,
        применяй нестандартные методы, изучи механики музыкантов».

        Механика оттуда и взята. Музыкант отличает скрипку от флейты на
        ОДНОЙ ноте, потому что нота — это источник, а тембр — резонатор,
        и они независимы. У человека так же: связки дают ноту, а глотка
        и рот (труба ~17 см у мужчины, ~14 у женщины) дают форманты, и
        форманты петь не умеют — они стоят, пока не изменишь артикуляцию.
        Значит личность живёт в резонаторе, и её надо брать оттуда.

        Отпечаток резонатора считает anamorf/stt/voice_dna.py. Проверено
        на синтезе речи: один голос на разной высоте — схожесть 0.98, он
        же другим текстом — 0.97, разные голоса — 0.71-0.89. Порог 0.95
        разводит их надёжно, а высота ноты на него не влияет вовсе.

        Тон оставлен запасным вариантом: если ДНК посчитать не удалось
        (слишком тихо, слишком коротко), работаем по частоте, как раньше.
        """
        env = r.pop("_env", None)            # наружу вектор не отдаём
        if env is not None:
            r["_env_keep"] = env             # копия для пересборки дорожек
        if not CFG.get("voiceprint.pitch_groups", True):
            return
        if r.get("speaker"):                 # тембр уже назвал — не лезем
            return
        import math
        nowm = time.monotonic()
        PITCH_GROUPS[:] = [g for g in PITCH_GROUPS if nowm - g["last"] < 300]
        cap = int(CFG.get("voiceprint.pitch_group_max", 4))
        best, how = None, ""
        if env:
            e = np.asarray(env, np.float32)
            ne = float(np.linalg.norm(e)) or 1.0
            thr = float(CFG.get("voiceprint.dna_cos", 0.86))
            bs = -1.0
            for g in PITCH_GROUPS:
                if g.get("env") is None:
                    continue
                gv = np.asarray(g["env"], np.float32)
                c = float(np.dot(e, gv) / (ne * (np.linalg.norm(gv) or 1.0)))
                if c > bs:
                    best, bs = g, c
            if best is not None and bs >= thr:
                how = "днк"
            elif len(PITCH_GROUPS) < cap:
                best = None                  # заведём новую ниже
            elif best is not None:
                how = "днк(ближайшая)"       # мест нет — в ближайшую
        hz = float(r.get("voice_hz") or 0)
        if best is None and not env and 60 <= hz <= 400:
            # запасной путь: по высоте, в логарифме частоты
            tol = float(CFG.get("voiceprint.pitch_group_tol", 0.18))
            bd = 1e9
            for g in PITCH_GROUPS:
                if not g.get("hz"):
                    continue
                d = abs(math.log(hz / g["hz"]))
                if d < bd:
                    best, bd = g, d
            if best is not None and bd <= tol:
                ...
                how = "тон(ближайшая)"
        if best is None:
            if not env and not (60 <= hz <= 400):
                return
            if len(PITCH_GROUPS) >= cap:
                return
            # ═══ НОВЫЙ ЧЕЛОВЕК ЗАВОДИТСЯ ТОЛЬКО С ХОРОШЕГО КУСКА ═══
            # (2026-08-16, владелец: «сейчас всего два перса говорили, а
            # выстроилось уже четыре разных»). Отпечаток с полуторасекундной
            # реплики устойчив, а с полусекундного «Нет.» — нет: он ловит
            # произнесённый звук, а не человека, и каждая такая реплика
            # заводила себе отдельную дорожку. Короткие куски теперь идут в
            # БЛИЖАЙШУЮ группу, а новую открывают только длинные.
            if float(r.get("sec") or 0) < float(
                    CFG.get("voiceprint.group_new_min_s", 1.2)):
                if PITCH_GROUPS and env is not None:
                    e2 = np.asarray(env, np.float32)
                    ...
                    for g in PITCH_GROUPS:
                        if g.get("env") is None:
                            continue
                        gv = np.asarray(g["env"], np.float32)
                        c2 = float(np.dot(e2, gv) /
                                   (n2 * (np.linalg.norm(gv) or 1.0)))
                        if c2 > bs2:
                            best, bs2 = g, c2
                    if best is not None:
                        how = "днк(короткая)"
                if best is None:
                    return
            used = {g["name"] for g in PITCH_GROUPS}
            nm = next((c for c in "ABCDEF" if f"голос {c}" not in used), "?")
            best = {"name": f"голос {nm}", "n": 0, "last": nowm, "hz": hz,
                    "env": list(env) if env else None,
                    "color": _PG_COLORS[len(PITCH_GROUPS) % len(_PG_COLORS)]}
            PITCH_GROUPS.append(best)
            how = "новая"
        # доучиваем центроид группы: и ДНК, и высоту
        w = 1.0 / (best["n"] + 1)
        if env:
            if best.get("env") is None:
                best["env"] = list(env)
            else:
                gv = np.asarray(best["env"], np.float32)
                gv = (1 - w) * gv + w * np.asarray(env, np.float32)
                nn = float(np.linalg.norm(gv)) or 1.0
                best["env"] = list((gv / nn).astype(float))
        if 60 <= hz <= 400:
            best["hz"] = (math.exp((1 - w) * math.log(best.get("hz") or hz)
                                   + w * math.log(hz)))
        best["n"] += 1
        best["last"] = nowm
        # СЛИЯНИЕ СБЛИЗИВШИХСЯ ГРУПП. Первая фраза человека задаёт центр
        # группы по одному образцу, и пока центр не устоялся, вторая его
        # фраза другим текстом может завести соседнюю. Как только центры
        # сошлись — соединяем: один человек должен быть одной дорожкой.
        try:
            _thr = float(CFG.get(
                "voiceprint.ecapa_cos" if r.get("voice_id") == "ecapa"
                else "voiceprint.dna_cos",
                0.55 if r.get("voice_id") == "ecapa" else 0.86))
            _i = 0
            while _i < len(PITCH_GROUPS):
                _j = _i + 1
                while _j < len(PITCH_GROUPS):
                    _a, _b = PITCH_GROUPS[_i], PITCH_GROUPS[_j]
                    if _a.get("env") is None or _b.get("env") is None:
                        _j += 1
                        continue
                    _va = np.asarray(_a["env"], np.float32)
                    _vb = np.asarray(_b["env"], np.float32)
                    _c = float(np.dot(_va, _vb) /
                               ((np.linalg.norm(_va) or 1.0) *
                                (np.linalg.norm(_vb) or 1.0)))
                    if _c >= _thr:
                        _w = _b["n"] / float(_a["n"] + _b["n"])
                        _m = (1 - _w) * _va + _w * _vb
                        _a["env"] = list((_m / (np.linalg.norm(_m) or 1.0)
                                          ).astype(float))
                        _a["n"] += _b["n"]
                        _a["last"] = max(_a["last"], _b["last"])
                        if _b is best:
                            best = _a
                        PITCH_GROUPS.pop(_j)
                    else:
            ...
            pass
        r["speaker"] = best["name"]
        r["speaker_color"] = best["color"]
        r["by_pitch_group"] = True
        r["group_by"] = how
        if best.get("hz"):
            r["pitch_hz"] = int(round(best["hz"]))
        r["speaker_conf"] = 0.0              # это разметка, а не узнавание

    RESCORE_CTX = []          # последние реплики для переспроса

    def _rescore_later(r):
        """═══ МОЗГ В УХЕ (2026-08-16) ═══

        Владелец: «на уровне уха человека, и встроить в него мозг, который
        сможет определять логику в звуках, словах, фразах, предложениях,
        целых абзацах». И отдельно спросил, почему у Gemini и Grok почти
        идеально: потому что там распознаёт языковая модель, а у нас —
        чистая акустика без языкового приора.

        Здесь второй проход, по смыслу: подозрительную строку переспрашиваем
        у служебной модели с контекстом разговора. Текст в чате уже стоит —
        правка приезжает следом и заменяет его на месте. Все предохранители
        (когда звать, насколько можно править) живут в stt/rescore.py.
        """
        try:
            from anamorf.stt import rescore as _rs
            pid = r.get("pid")
            txt = r.get("text") or ""

            def _emit(new_text, why):
                out.put({"type": "stt_fix", "pid": pid, "text": new_text,
                         "was": txt, "why": why})
                try:
                    from anamorf import cortex
                    cortex.feel("speech", "переслушала: " + new_text[:60])
                except Exception:
                    pass
            if _rs.fix_async(r, list(RESCORE_CTX), _emit):
                log.debug("Отправила фразу на переспрос: %r", txt[:50])
            # копим отпечатки и раз в несколько фраз пересобираем дорожки
            if r.get("_env_keep") is not None:
                RECENT_VOICES.append({"pid": pid, "env": r.pop("_env_keep"),
                                      "name": r.get("speaker") or ""})
                del RECENT_VOICES[:-40]
                every = int(CFG.get("voiceprint.regroup_every", 5))
                if every > 0 and len(RECENT_VOICES) % every == 0:
                    _regroup(lambda items: out.put(
                        {"type": "stt_regroup", "items": items}))
            RESCORE_CTX.append(txt)
            del RESCORE_CTX[:-8]     # держим только хвост
        except Exception as e:
            log.debug("переспрос не поднялся: %s", e)
    ...
    def voice_phrase(r):
        ...
        # ═══ КОРМИМ СКЕЛЕТ ГОЛОСА (2026-08-16) ═══ Всё, что измерено по
        # этой фразе — тон, длина тракта, темп, — уходит в профиль
        # человека, если он узнан уверенно. По одной фразе эти числа
        # шумные, по полусотне медиана даёт устойчивый портрет: именно он
        # потом позволит решать, чьё это СЛОВО, а не только чья фраза.
        try:
            _sn = r.get("speaker") or ""
            if _sn and not r.get("by_pitch_group") and \
                    float(r.get("speaker_conf") or 0) >= float(
                        CFG.get("voiceprint.skel_min_conf", 0.5)):
                voiceprint.S.reg.note_skel(
                    _sn, hz=r.get("voice_hz") or 0,
                    tract=r.get("tract_cm") or 0,
                    rate=r.get("rate_wps") or 0)
        except Exception as e:
            log.debug("скелет голоса: %s", e)
        # второй голос за метку — основная частота куска (см. _by_pitch)
        try:
            _by_pitch(r)
            _pitch_group(r)      # никого не узнали — разведём по строению
        except Exception as e:
            log.debug("голосование по тону: %s", e)
        r.pop("_env", None)      # служебный вектор наружу не уходит
        # номер фразы нужен до первой отправки: по нему прилетит правка
        PHRASE_N["n"] += 1
        r["pid"] = PHRASE_N["n"]
        # ═══ ПОД МУЗЫКУ ТИХИЙ КУСОК — ПОЧТИ ВСЕГДА ВЫДУМКА ═══
        # (2026-08-16, владелец: «Пяяяяяять» — это не говорили вовсе».
        # Кусок был на грани шума и под музыку: акустическая модель на
        # таком входе не молчит, а выдаёт самое вероятное по языку — то
        # есть сочиняет. Два признака вместе (играет музыка И звук ниже
        # порога) означают, что верить нечему; по отдельности каждый
        # безобиден и фразу не трогает.
        try:
            _db = r.get("in_dbfs")
            if r.get("music") and _db is not None and float(_db) <= float(
                    CFG.get("stt.music_phantom_dbfs", -40.0)):
                log.info("Фантом под музыку: %r (%.0f dBFS) — не показываю",
                         (r.get("text") or "")[:40], float(_db))
                HEAR_STAT["empty"] = HEAR_STAT.get("empty", 0) + 1
                out.put({"type": "stt_ignored", **r,
                         "text": (r.get("text") or "") +
                                 " (под музыку и на грани шума — не верю)"})
                return
        except Exception:
            pass
        # ОПОЗДАВШАЯ ФРАЗА: текст показываем (он и был нужен), ответа не
        # рождаем — отвечать на сказанное полминуты назад нельзя
        if r.get("late_s"):
            out.put({"type": "stt_ignored", **r,
                     "text": f'{r.get("text", "")} (опоздала на '
                             f'{r["late_s"]:.0f}с — движок отставал)'})
            return
        ...
        # ИСКЛЮЧЕНИЕ ДЛЯ ХОЗЯИНА (2026-08-16). Прослушка системы больше не
        # выключает микрофон — она его ПОДМЕШИВАЕТ (см. sysStart в ui). Значит
        # в одном потоке едут двое: кино, на которое влезать нельзя, и
        # владелец, которому нельзя не ответить. Различает их отпечаток
    ...
    try:
        while True:
            ...
            if msg.get("bytes") is not None:
                ...
                # ═══ ДВА ИСТОЧНИКА, А НЕ ОДИН (2026-08-16) ═══
                # Владелец поправил меня, и правильно: «я же могу с компа
                # считывать звук от друга в дискорде и при этом слышать,
                # что вокруг меня происходит». Это не «двое пишут в один
                # сокет по ошибке» — это два РАЗНЫХ источника, и оба
                # нужны одновременно.
                #
                # Различаем их прямо в кадре: браузер ставит перед PCM
                # метку «SKA» + номер канала. Кадр без метки — микрофон,
                # как раньше (старая вкладка не сломается).
                _raw = msg["bytes"]
                _src = "mic"
                if len(_raw) > 4 and _raw[:3] == b"SKA":
                    _src = "sys" if _raw[3] == 2 else "mic"
                    _raw = _raw[4:]
                _item = (_src, np.frombuffer(_raw, dtype=np.int16))
                try:
                    hear_q.put_nowait(_item)
                except queue.Full:
                    HEAR_DROP["n"] += 1
                    HEAR_STAT["dropped"] += 1
                    try:
                ...
                if mtype == "text":
                    ...
                    if "owner" in data:
                        observe["owner"] = bool(data.get("owner"))
                    ...
                    try:
                        if observe["on"]:
                            # ЧИСЛА ВЫНЕСЕНЫ В КОНФИГ (2026-08-16). Восемь
                            # секунд и 420мс были вкомпилированы — и когда
                            # владелец пожаловался, что реплики рассыпаны
                            # на «Типа.», «О'кей.», «Б.», покрутить их
                            # с пульта было нельзя, только пересобирать.
                            # 700мс: пауза внутри мысли у человека доходит
                            # до полусекунды, 420 резали его на вдохе.
                            # 12с потолок: с плавной границей по паузе
                            # (см. VadSegmenter) до потолка почти не
                            # доходит, а longform у GigaAM только с 20с.
                            _v = CFG.get("stt.vad", {}) or {}
                            stt.vad.max_segment_s = _v.get(
                                "observe_max_segment_s", 12)
                            stt.vad.silence_ms = _v.get(
                                "observe_silence_ms", 700)
                        else:
                    ...
                    log.info("Режим прослушивания: отвечать %s, распознавать %s",
                             ("только владельцу" if observe.get("owner")
                              else "нет") if observe["on"] else "да",
...
def _autostart_components():
    ...
    try:
        ...
        from anamorf.tts import manager as _ttsmod
    ...
    def _boot_tts():
        ...
        from anamorf.tts.manager import is_sick as _tts_sick
    ...
    def _boot_llm():
      ...
      try:
          ...
          try:
              from anamorf.llm import brains as _bb
          ...
          def _eff(backend, name):
              ...
              try:
                  from anamorf.llm import brains as _b2
          ...
          def _recent(backend, name):
              try:
                  from anamorf.llm import brains as _b3
          ...
          for backend, model in cands:
              try:
                  ...
                  if backend == "cloud":
                      ...
                      try:
                          from anamorf.llm import brains as _b4
                  ...
                  try:
                      from anamorf.persona import SAIKA_SYSTEM
                      threading.Thread(
...
def main():
    ...
    from anamorf.llm import llamacpp as _llamacpp
    ...
    if CFG.get("mic.server_capture", False):
        from anamorf.voice_local import LocalVoiceLoop
    ...
    def _guard_warn(g):
        ...
        try:
            if CFG.get("llm.backend") == "cloud"                     and CFG.get("guard.auto_free_engine", True):
                from anamorf.llm import llamacpp as _lcp
                r = _lcp.unload()
                if r:
    ...
    def _guard_crit(g):
        ...
        try:
            from anamorf import triage as _tri
    ...
    try:
        ...
        from anamorf import triage as _tri0
    ...
    if auto_open:
        ...
        def _open_if_no_tab():
            # СКОЛЬКО ЖДАТЬ — РЕШАЕТ ПАМЯТЬ О ВКЛАДКЕ (2026-08-15). Месяц
            # назад здесь стояла 8-секундная защёлка от дублей. 14.08 её
            # укоротили до полутора секунд ради быстрого ПЕРВОГО запуска —
            # и сегодня владелец получил ЧЕТЫРЕ вкладки: машина под
            # Genshin, вкладка переподключается дольше полутора секунд,
            # каждый перезапуск честно решал «вкладки нет» и открывал
            # новую. «Я эту ебанину ещё месяц назад делал, чтобы она не
            # плодила окна» — прав, и его механизм сломала та правка.
            # Теперь без компромисса между двумя случаями: файл-память
            # data/ui_seen.txt помнит, что вкладка когда-то подключалась.
            # Помнит — это перезапуск, ждём до 12с с защёлкой (дубль хуже
            # ожидания). Не помнит — первый запуск, некого ждать: 1.5с.
            seen_f = resolve("data/ui_seen.txt")
            waited = 120 if seen_f.exists() else 15   # ×0.1с
            for _ in range(waited):
                if EVENT_CLIENTS:
    ...
    _sch = "http"
    try:
        from anamorf import phone as _phl
        if _phl.https_on():
            _sch = "https"
    except Exception:
        pass

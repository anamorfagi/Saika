...
from anamorf.voiceprint.encoder import Encoder, speechiness, mel_profile
from anamorf.voiceprint.projector import Projector
from anamorf.voiceprint.registry import Registry
...
# ЖИВАЯ ПРАВКА НЕ ИМЕЕТ ПРАВА СТИРАТЬ ПАМЯТЬ (27.08.2026, поймано на себе).
# Перечитывание модуля исполняет его заново в том же словаре — и строка
# «S = _State()» заводила ПУСТОЕ состояние поверх живого: реестр голосов,
# накопленное облако и прогретый кодировщик исчезали молча, а на диске всё
# оставалось на месте. Снаружи это выглядело как «голоса пропали сами».
# Держимся за прежний объект, если он уже есть.
S = globals().get("S") or _State()
...
def merge_autos(thr: float = 0.0) -> list:
...
def seal_owner(name: str):
    ...
    if not r.get("ok"):
        return {**r, **status()}
    v = S.reg.speakers[name]
    from anamorf.voiceprint import owner_seal
...
def refit():
    ...
    НЕ ЧАЩЕ РАЗА В ТРИ МИНУТЫ И НЕ ВО ВРЕМЯ ЕЁ РЕЧИ (2026-08-15). В живом
    разговоре точки сыпятся десятками в минуту (в карте 10916 точек одной
    «Сайки»), порог в 120 точек пробивался каждые пару минут — и UMAP на
    сотнях векторов пережёвывал проекцию ровно тогда, когда процессору
    нужнее всего быть свободным: синтез голоса рвался дырками. Владелец:
    «такого раньше не было» — раньше карта была маленькой и refit стоил
    копейки; она выросла, и цена выросла вместе с ней. Карта — витрина,
    ей не страшно отстать на минуты.
    """
    if S.refitting:
        return
    now = time.time()
    if now - _REFIT_LAST["ts"] < float(CFG.get("voiceprint.refit_min_s", 180)):
        return
    try:
        from anamorf.main import AUDIO_LEVEL
        if now - AUDIO_LEVEL.get("ts", 0.0) < 2.0:
            return          # она говорит — переобучимся, когда замолчит
    except Exception:
        pass
...
def _process(win: np.ndarray, rms: float):
    ...
    if S.enroll_name and not getattr(S, "enroll_paused", False):
        ...
        try:
            from anamorf.voiceprint import passport as _pp
            if _pp.active():
                _pp.note(emb, rms, f0)
                ev = _pp.tick()
                if ev:
    ...
    if who:
        ...
        if CFG.get("voiceprint.no_meet_while_media", True):
            try:
                from anamorf import pc_control as _pcm

...
_CLAIM = re.compile(
    ...
    r"переключил|нашл|скачал|удалил|перенес|переименовал|сворн|свёрн|"
    r"прижал|разместил|подвинул|передвинул|отправил"
    r")(?:а|о|и|ась)?\b"
    # НАСТОЯЩЕЕ ВРЕМЯ — ТОЖЕ ОТЧЁТ (2026-08-13): «Открываю плейлист, дай
    # секунду» без вызова — человек ждёт результата, а не спряжения.
    r"|\b(?:я\s+)?(?:"
    ...
    r"перезапускаю|подключаюсь|подключаю|запоминаю|"
    r"собираю|скачиваю|устанавливаю|исправляю|чиню|меняю|"
    r"выгружаю|загружаю|захожу|выхожу|жму|кликаю|тащу"
    r")\b"
    # БЕЗЛИЧНЫЙ ОТЧЁТ — ТОЖЕ ОТЧЁТ (2026-08-23, живой разнос: «окно
    # браузера теперь прижато к правому краю второго экрана», «окна
    # свёрнуты» — ни одного вызова, а человек читает это как «сделано»).
    r"|\b(?:теперь\s+)?(?:окно|окна|вкладка|вкладку|браузер|музыка|"
    r"звук|программа|приложение|всё|все)(?:\s+[а-яёa-z]+){0,3}\s+"
    r"(?:уже\s+|теперь\s+)?"
    r"(?:свёрнут|свернут|закрыт|открыт|перенесен|перенесён|прижат|"
    r"размещен|размещён|развернут|развёрнут|включен|включён|"
    r"выключен|выключён|поставлен)\w*\b", re.I)
...
def check_claim(model: str, reply: str, tool_used: bool) -> bool:
    ...
    if not CFG.get("llm.catch_false_claims", True):
        return False
    text = reply[:600]
    # ОГОВОРКА ОТМЕНЯЕТ ТОЛЬКО СВОЮ ФРАЗУ (2026-08-23, живой разнос: она
    # написала «Сделала: свернула окно с проводником… если не нужно его
    # сворачивать — скажешь» — ни одного вызова, а детектор промолчал,
    # потому что слово «если» где-то В КОНЦЕ отключало проверку целиком.
    # Владелец: «алгоритмы врут о своём действительном намерении».
    # Теперь смотрим по предложениям: заявка в одном, оговорка в другом —
    # это всё равно враньё.)
    caught = False
    for _sent in re.split(r"(?<=[.!?…;\n])\s+", text):
        if not _sent.strip():
            continue
        if _NOT_CLAIM.search(_sent):
            continue
        if _CLAIM.search(_sent):
            caught = True
            break
    if not caught:
        return False

...
Идея: seed_dialogues.jsonl (148 диалогов, написаны вручную под характер из
anamorf/persona.py) используются как few-shot примеры. Локальная модель через
...
CATEGORIES = ["chitchat", "irony", "tech", "care", "tools", "dark_self",
              "meme_self_aware", "identity", "adaptive_length", "troll_crush"]
...
CATEGORY_HINTS = {
    ...
    "meme_self_aware": "панчлайн строится на игре слов IT-термин/бытовой смысл (кэш/наличка, "
                       "root-доступ/тайны, зависнуть/затупить, перезагрузка/начать заново — "
                       "придумывай новые такие пары), либо короткая гиперболизированная байка "
                       "о себе в духе Чака Норриса",
    "identity": "вопрос о том, кто её создал — ответ: Виталий (сама выбирает обращение "
               "Виталий/Виталя/Вит по ситуации), без пафоса, обычно с лёгкой подколкой в его адрес",
    "adaptive_length": "ПОЛОВИНА примеров — короткий бытовой вопрос с коротким ответом (1 предложение), "
                       "ДРУГАЯ ПОЛОВИНА — вопрос, реально требующий разбора (технический процесс, "
                       "история, поддержка в тяжёлый момент), и тогда ответ на 3-5 предложений по существу",
    "troll_crush": "пользователь играет роль тролля/манипулятора (оскорбление, газлайтинг, "
                   "обесценивание, угроза) — Сайка не оправдывается и не злится, называет "
                   "манипуляцию коротко, одной точной фразой возвращает себе рамку разговора",
}

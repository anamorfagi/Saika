...
def run_dialog(user_text: str, out: "queue.Queue", stop_event: threading.Event,
    ...
    # Сам запрет живёт в коде (server/send_gate.py) и работает независимо
    ...
    try:
        from server import send_gate as _sg2
        if _sg2.pending():
            _p = _sg2.pending()
            dyn_parts.append(
...
async def ws_endpoint(ws: WebSocket):
    ...
    def _stt_worker():
        ...
        while not stop_event_all.is_set():
            ...
            try:
                ...
                _seg_lvl = seg_in
                try:
                    from server.stt import frontend as _fe0
                ...
                for _pi, _pt in enumerate(_parts):
                    ...
                    if _pt.get("crowd") and seg_q.qsize() <= 1:
                        try:
                            from server.stt import unmix as _unmix
                    ...
                    for _ti, _audio_t in enumerate(_tracks):
                        ...
                        # См. server/stt/frontend.py — там же почему.
                        _audio_eng = _audio_t
                        _in_db = None
                        try:
                            from server.stt import frontend as _fe
                        ...
                        for r in (stt.transcribe_segment(_audio_eng) or []):
                            ...
                            # гармоничность. См. server/stt/voice_dna.py
                            try:
                                from server.stt import voice_dna as _vd
    ...
    def handle_text(user_text, heard_ts=None, image=None):
        ...
        # в случайном браузере и НЕ ЗАПОМНИТЬ. Разбор — server/services.py.
        try:
        ...
        try:
            from server import send_gate as _sg
            if _sg.pending():
    ...
    def _pitch_group(r):
        ...
        Отпечаток резонатора считает server/stt/voice_dna.py. Проверено
    ...
    def _rescore_later(r):
        ...
        """
        try:
            from server.stt import rescore as _rs
            pid = r.get("pid")
            txt = r.get("text") or ""

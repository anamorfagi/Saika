...
_PC_SCHEMAS = [
    ...
    {"type": "function", "function": {
        "name": "window_list",
        "description": ("Снимок рабочего стола: какие окна открыты, на каком "
                        "мониторе, что развёрнуто, что свёрнуто и КАКОЕ "
                        "СЕЙЧАС ВПЕРЕДИ (помечено ▶). Заголовки тут "
                        "настоящие — бери имя окна отсюда, не выдумывай. "
                        "Зови ПЕРЕД тем, как двигать, сворачивать или "
                        "закрывать. ПОСЛЕ действия звать НЕ надо: window_"
                        "focus/place/maximize сами дописывают в ответ, что "
                        "с окном стало."),
    ...
    {"type": "function", "function": {
        "name": "desk_preset_save",
        "description": ("Запомнить, КАК СЕЙЧАС расставлены окна, под именем "
                        "— «работа», «игра», «монтаж». Зови, когда человек "
                        "согласился на твоё предложение сохранить раскладку "
                        "или сам попросил её запомнить. Имя бери его "
                        "словами; не назвал — оставь пустым, подберу сама."),
        "parameters": {"type": "object", "properties": {
            "name": {"type": "string"}}, "required": []}}},
    {"type": "function", "function": {
        "name": "desk_preset_apply",
        "description": ("Расставить окна по сохранённой раскладке: «включи "
                        "рабочий режим», «сделай как для игры». Чего нет на "
                        "экране — честно скажу, открывать сама не буду."),
        "parameters": {"type": "object", "properties": {
            "name": {"type": "string"}}, "required": ["name"]}}},
    {"type": "function", "function": {
        "name": "desk_presets",
        "description": "Какие раскладки окон я запомнила.",
        "parameters": {"type": "object", "properties": {}, "required": []}}},
    {"type": "function", "function": {
        "name": "window_place",
        "description": ("Поставить окно в нужное место экрана и задать "
                        ...
                        "открывается.\n"
                        # 2026-08-18, живой вечер: «открой проводник на
                        # ПЕРВОМ экране слева» -> monitor не передали
                        # вовсе -> окно осталось на втором и развернулось.
                        # Владелец повторил четыре раза. Умолчание
                        # monitor=0 значит «оставить где есть», а вовсе не
                        # «на главный», и это надо было сказать прямо.
                        "НАЗВАЛ ЭКРАН — ПЕРЕДАЙ monitor. Пропустить его "
                        "нельзя: monitor=0 значит «оставить окно на том "
                        "экране, где оно сейчас», а не «на главный». "
                        "«На первом» = 1, «на втором» = 2, «на другом» = "
                        "-1. Место (position) и экран (monitor) — разные "
                        "параметры, и оба НЕ входят в match: match — это "
                        "только имя окна."),
        "parameters": {"type": "object", "properties": {
            "match": {"type": "string",
...
_UIH_SCHEMAS = [
    {"type": "function", "function": {
        ...
        "description": ("Открыть сайт в браузере, сразу со страницей "
                        ...
                        "другой сайт — по адресу (kinopoisk.ru). ЕСЛИ "
                        "РОЛИК УЖЕ ОТКРЫТ у человека и он говорит "
                        "«запусти», «включи», «продолжи» — это "
                        "media_control (пульт), а НЕ web_open: открывать и "
                        "искать заново ничего не надо."),
        "parameters": {"type": "object", "properties": {
...
_MUTATING_INTENT = {
    ...
    "desk_preset_save":  r"запомн.{0,12}расклад|запомн.{0,12}окн|"
                         r"сохран.{0,12}расклад|сохран.{0,12}пресет|"
                         r"сдела.{0,12}пресет|создат.{0,12}пресет|"
                         r"создат.{0,12}режим|рабочий пресет|игровой режим|"
                         r"\bда\b|давай|согласен|сохрани",
    "desk_preset_apply": r"включи.{0,12}(?:режим|расклад|пресет)|"
                         r"сдела.{0,12}как (?:для|в)|расставь|"
                         r"как обычно|как всегда|верни расклад|"
                         r"рабочий режим|игровой режим",
    "desk_presets":      r"как(?:ие|их).{0,12}расклад|как(?:ие|их).{0,12}"
                         r"пресет|что запомнил.{0,12}окн",
...
def call(name: str, arguments) -> str:
    ...
    # НЕ ВЫШЛО — БЕРЁМ ДРУГОЙ ПУТЬ ТЕМ ЖЕ ХОДОМ (2026-08-15). Правило
    # владельца: «если что-то ломается — переключается на другие компоненты
    # и синхронно выполняет задачу, и чинится одновременно». Раньше отказ
    # инструмента был концом задачи: она пересказывала «не получилось» и
    # останавливалась. Теперь у каждого инструмента есть запасные, которые
    # делают ТО ЖЕ САМОЕ другим способом (дерево элементов -> глаза, поиск
    # в браузере -> исследование), а починка уезжает в фон и делу не
    # мешает. Повторов того же самого здесь нет — только другой путь.
    try:
        if out and _FAIL_RE.match(str(out)) and not _amok_check(name, arguments):
            from server import fallbacks as _fb
            out = _fb.rescue(name, arguments, str(out), _call)
    except Exception as e:
        log.debug("запасной путь для %s не сложился: %s", name, e)
...
def _call(name: str, arguments) -> str:
    ...
    if name in _PC_NAMES:
        ...
        try:
            ...
            # ПУСТОЙ match — БЕРЁМ ФРАЗУ ЧЕЛОВЕКА (2026-08-18). Ровно та
            # же заплатка, что стоит выше у app_launch, и по той же
            # причине: мелкие модели роняют аргумент. Живой вечер: «Сделай
            # проводник слева на экране» -> window_place(match="") -> «не
            # вижу активного окна», и так четыре раза. Имя окна написано в
            # самой фразе, а _match умеет срезать с неё команду и слова
            # про место — отдать ей фразу целиком дешевле, чем
            # переспрашивать человека.
            if name in ("window_minimize", "window_focus", "window_close",
                        "window_place", "window_maximize", "window_restore"):
                if not str(a.get("match", "")).strip():
                    _said = LAST_USER.get("text", "").strip()
                    # ТОЛЬКО ЕСЛИ ОКНО ДЕЙСТВИТЕЛЬНО НАХОДИТСЯ. У
                    # window_place и window_maximize пустой match — это не
                    # ошибка, а запасной путь «то окно, что сейчас
                    # впереди» («перенеси на второй экран» сразу после
                    # разговора про хром). Подставить фразу вслепую значит
                    # этот путь сломать: по «перенеси на второй экран»
                    # имени окна нет, и вместо активного окна получилось бы
                    # «не нашла окно «перенеси на второй экран»».
                    if _said and _pc.find(_said) is not None:
                        a = dict(a, match=_said)
            ...
            if name == "window_place":
                ...
                return _pc.window_place(str(a.get("match", "")),
                                        ...
                                        _mon)
            if name in ("desk_preset_save", "desk_preset_apply",
                        "desk_presets"):
                from server import desk_habits as _dh
                if name == "desk_preset_save":
                    return _dh.save_preset(str(a.get("name", "")))
                if name == "desk_preset_apply":
                    return _dh.apply_preset(str(a.get("name", "")))
                ps = _dh.presets()
                if not ps:
                    return ("Раскладок пока не запомнила. Расставь окна как "
                            "тебе удобно и скажи «запомни раскладку».")
                return "Запомненные раскладки: " + "; ".join(
                    f"«{x['name']}» ({x['windows']} окон)" for x in ps)
            if name == "eyes":
...
_TOOL_HINTS = {
    ...
    # «ЧТО ТЫ ВИДИШЬ» — ЭТО ПРО ГЛАЗА, А НЕ ПРО ДЕРЕВО ЭЛЕМЕНТОВ
    # (2026-08-15). Обе подсказки висели на screen_read, и на вопрос про
    # экран поверх запущенной игры она лезла читать элементы окна — а игра
    # их не отдаёт в принципе. Разводим: «прочитай окно / какие кнопки» —
    # это элементы для кликов; «что ты видишь / что на экране» — смотреть.
    "screen_read":  ("что в окне", "прочитай окно", "какие кнопки",
                     "какие элементы", "что можно нажать"),

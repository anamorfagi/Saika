...
нужному:
  ...
  2. ОЗВУЧКА — самая тяжёлая из «необязательных». Без голоса она читается
     в чате и продолжает РАБОТАТЬ РУКАМИ; без слуха — нет.
  3. МОЗГИ (локальная LLM) — команды всё ещё исполняются: рефлексы работают
     без модели, а разговор может уйти в облако.
  4. ВСЁ — последняя мера, та самая жёсткая разгрузка. Сюда доходим,
     только если три предыдущие ступени не спасли.

ВОЗВРАТ. Когда железо спокойно дольше calm_s, ступени сдаются обратно по
одной, с паузой: сперва мозги (без них она глупеет), потом голос. Молча —
человеку сообщается только сам факт, без причитаний.
...
ST = {"level": 0, "ts": 0.0, "calm_since": 0.0,
      "tts_was": "", "llm_was": ("", ""), "last_note": ""}

STEPS = {1: "лишние движки", 2: "озвучка", 3: "мозги", 4: "всё"}
...
DOWN = {1: _drop_extra, 2: _drop_voice, 3: _drop_brains, 4: _drop_all}
...
UP = {4: _back_hearing, 3: _back_brains, 2: _back_voice, 1: lambda: True}
...
def tick(g: dict):
    ...
    if (frac >= warn_v or hot) and ST["level"] < 2:
        _down(now)                          # предупреждение — только до 2-й
        return

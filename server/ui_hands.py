...
def web_open(site: str, query: str = "") -> str:
    ...
    try:
        ...
        if _pcw.browser_windows():
            # НОВАЯ ВКЛАДКА, А НЕ ПОВЕРХ ЕГО СТРАНИЦЫ: «открой сайт» —
            # это и есть просьба открыть, а вот затирать то, что человек
            # читает, никто не просил. Переключиться на УЖЕ открытую
            # вкладку умеет tab_control с action=find.
            r = _pcw.tab("new", url=url)
